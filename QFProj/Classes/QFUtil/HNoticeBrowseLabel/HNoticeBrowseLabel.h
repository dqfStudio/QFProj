...
typedef NS_ENUM(NSInteger, HNoticeBrowseMode) {
    // 公告标题数组播放模式 只有一个标题时，静止不播放
    HNoticeBrowseDefalut = 0,
    
    // 公告标题数组播放模式 只有一个标题时，横向播放
    HNoticeBrowseHorizontalScrollWhileSingle = 1,
    
    // 公告标题数组播放模式 有多个标题时，向上滚动播放
    HNoticeBrowseVerticalScrollWhileMore = 2,
};

@interface HNoticeBrowseLabel : UIView
...
// 公告标题数组播放模式（只有一个标题时，静止不播放，或横向播放；有多个标题时，向上滚动播放）
@property (nonatomic, assign) HNoticeBrowseMode browseMode;
// 公告内容字体大小（默认13.0）
...
// 后台设置的循环次数
@property (nonatomic, assign) NSInteger limitLoopNum;
@end
