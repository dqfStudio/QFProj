...
@interface HNoticeBrowseLabel () <UIScrollViewDelegate>
...
/// 后台设置的循环次数
@property (nonatomic, assign) NSInteger loopNum;
@end

@implementation HNoticeBrowseLabel
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        _textDuration = 8.0;
        _limitLoopNum = 100000000;
        _loopNum = 0;
        _browseMode = HNoticeBrowseHorizontalScrollWhileSingle;
        _showMoreButton = NO;
    }
    return self;
}
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    [self textAnimationStart];
    if (self.textClick) {
        self.textClick(self.textIndex > 0?(self.textIndex - 1):0);
    }
}
...
- (void)viewAnimationMoveSingleRow {
    ...
    [UIView animateWithDuration:_textDuration - 1 delay:_delayTime options:UIViewAnimationOptionCurveLinear animations:^{
        // 位移
...
- (void)textAnimationStart {
    if (_textAnimationPauseWhileClick) {
        if (self.isSingleRow) {
            if (_browseMode == HNoticeBrowseHorizontalScrollWhileSingle) {
                [self playlayer:self.label01.layer];
                [self playlayer:self.label02.layer];
            }
        }else {
            if (_browseMode == HNoticeBrowseVerticalScrollWhileMore) {
                if (self.timer) {
...
- (void)startTimer {
    ...
    if (_browseMode == HNoticeBrowseDefalut) {
        return;
    }else if (_browseMode == HNoticeBrowseHorizontalScrollWhileSingle) {
        if (self.isSingleRow) {
    ...
    }else if (_browseMode == HNoticeBrowseVerticalScrollWhileMore) {
        if (self.isSingleRow) {
            return;
        }else {
