...
- (void)beginImageWithView:(UIView *)view {
    NSString *imagePath = [NSString stringWithFormat:@"/documents/snapshotImages/%@.png", NSStringFromClass(self.class)];
    if (![self.allVCViews containsObject:imagePath]) {
        [self.allVCViews addObject:imagePath];
        UIGraphicsBeginImageContext(view.frame.size);
        [view.layer renderInContext:UIGraphicsGetCurrentContext()];
        UIImage *viewImage = UIGraphicsGetImageFromCurrentImageContext();
        NSData *data = UIImagePNGRepresentation(viewImage);
        imagePath = [NSHomeDirectory() stringByAppendingString:imagePath];
