...
- (void)connection:(NSURLConnection *)connection didSendBodyData:(NSInteger)bytesWritten totalBytesWritten:(NSInteger)totalBytesWritten totalBytesExpectedToWrite:(NSInteger)totalBytesExpectedToWrite {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(kk_upLoad:progress:)]) {
        
        [self.delegate kk_upLoad:self progress:1 - (CGFloat)bytesWritten/(CGFloat)totalBytesWritten];
    }
}

- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error {
    
    if (self.delegate && [self.delegate respondsToSelector:@selector(kk_upLoad:failedWithError:)]) {
        
        [self.delegate kk_upLoad:self failedWithError:error];
    }
}
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(kk_upLoad:finishedWithDict:)]) {
        
        [self.delegate kk_upLoad:self finishedWithDict:dict];
    }
}
