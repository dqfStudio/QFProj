...
+ (HKeyChainStore *)keyChainStoreWithService:(NSString *)service {
    return [[self alloc] initWithService:service accessGroup:nil];
}

+ (HKeyChainStore *)keyChainStoreWithService:(NSString *)service accessGroup:(NSString *)accessGroup {
...
+ (HKeyChainStore *)keyChainStoreWithServer:(NSURL *)server protocolType:(HKeyChainStoreProtocolType)protocolType {
    return [[self alloc] initWithServer:server protocolType:protocolType authenticationType:HKeyChainStoreAuthenticationTypeDefault];
}
...
- (instancetype)initWithService:(NSString *)service accessGroup:(NSString *)accessGroup {
    self = [super init];
    if (self) {
        _itemClass = HKeyChainStoreItemClassGenericPassword;
...
- (instancetype)initWithServer:(NSURL *)server protocolType:(HKeyChainStoreProtocolType)protocolType {
    return [self initWithServer:server protocolType:protocolType authenticationType:HKeyChainStoreAuthenticationTypeDefault];
}
...
- (instancetype)initWithServer:(NSURL *)server protocolType:(HKeyChainStoreProtocolType)protocolType authenticationType:(HKeyChainStoreAuthenticationType)authenticationType {
    self = [super init];
    if (self) {
        _itemClass = HKeyChainStoreItemClassInternetPassword;
...
- (void)commonInit {
    _accessibility = HKeyChainStoreAccessibilityAfterFirstUnlock;
...
+ (NSString *)stringForKey:(NSString *)key service:(NSString *)service accessGroup:(NSString *)accessGroup error:(NSError *__autoreleasing *)error {
    ...
    HKeyChainStore *keychain = [HKeyChainStore keyChainStoreWithService:service accessGroup:accessGroup];
...
+ (NSArray H_KEY_TYPE *)allKeysWithItemClass:(HKeyChainStoreItemClass)itemClass {
    CFTypeRef itemClassObject = kSecClassGenericPassword;
    if (itemClass == HKeyChainStoreItemClassGenericPassword) {
        itemClassObject = kSecClassGenericPassword;
    } else if (itemClass == HKeyChainStoreItemClassInternetPassword) {
...
+ (NSArray *)allItemsWithItemClass:(HKeyChainStoreItemClass)itemClass {
    CFTypeRef itemClassObject = kSecClassGenericPassword;
    if (itemClass == HKeyChainStoreItemClassGenericPassword) {
        itemClassObject = kSecClassGenericPassword;
    } else if (itemClass == HKeyChainStoreItemClassInternetPassword) {
...
+ (void)requestSharedWebCredentialWithCompletion:(void (^)(NSArray H_CREDENTIAL_TYPE *credentials, NSError *error))completion {
    [self requestSharedWebCredentialForDomain:nil account:nil completion:completion];
}

+ (void)requestSharedWebCredentialForDomain:(NSString *)domain account:(NSString *)account completion:(void (^)(NSArray H_CREDENTIAL_TYPE *credentials, NSError *error))completion {
...
- (CFTypeRef)itemClassObject {
    switch (_itemClass) {
        case HKeyChainStoreItemClassGenericPassword:
            return kSecClassGenericPassword;
        case HKeyChainStoreItemClassInternetPassword:
...
- (CFTypeRef)protocolTypeObject {
    switch (_protocolType) {
        case HKeyChainStoreProtocolTypeFTP:
            return kSecAttrProtocolFTP;
        case HKeyChainStoreProtocolTypeFTPAccount:
            return kSecAttrProtocolFTPAccount;
        case HKeyChainStoreProtocolTypeHTTP:
            return kSecAttrProtocolHTTP;
        case HKeyChainStoreProtocolTypeIRC:
            return kSecAttrProtocolIRC;
        case HKeyChainStoreProtocolTypeNNTP:
            return kSecAttrProtocolNNTP;
        case HKeyChainStoreProtocolTypePOP3:
            return kSecAttrProtocolPOP3;
        case HKeyChainStoreProtocolTypeSMTP:
            return kSecAttrProtocolSMTP;
        case HKeyChainStoreProtocolTypeSOCKS:
            return kSecAttrProtocolSOCKS;
        case HKeyChainStoreProtocolTypeIMAP:
            return kSecAttrProtocolIMAP;
        case HKeyChainStoreProtocolTypeLDAP:
            return kSecAttrProtocolLDAP;
        case HKeyChainStoreProtocolTypeAppleTalk:
            return kSecAttrProtocolAppleTalk;
        case HKeyChainStoreProtocolTypeAFP:
            return kSecAttrProtocolAFP;
        case HKeyChainStoreProtocolTypeTelnet:
            return kSecAttrProtocolTelnet;
        case HKeyChainStoreProtocolTypeSSH:
            return kSecAttrProtocolSSH;
        case HKeyChainStoreProtocolTypeFTPS:
            return kSecAttrProtocolFTPS;
        case HKeyChainStoreProtocolTypeHTTPS:
            return kSecAttrProtocolHTTPS;
        case HKeyChainStoreProtocolTypeHTTPProxy:
            return kSecAttrProtocolHTTPProxy;
        case HKeyChainStoreProtocolTypeHTTPSProxy:
            return kSecAttrProtocolHTTPSProxy;
        case HKeyChainStoreProtocolTypeFTPProxy:
            return kSecAttrProtocolFTPProxy;
        case HKeyChainStoreProtocolTypeSMB:
            return kSecAttrProtocolSMB;
        case HKeyChainStoreProtocolTypeRTSP:
            return kSecAttrProtocolRTSP;
        case HKeyChainStoreProtocolTypeRTSPProxy:
            return kSecAttrProtocolRTSPProxy;
        case HKeyChainStoreProtocolTypeDAAP:
            return kSecAttrProtocolDAAP;
        case HKeyChainStoreProtocolTypeEPPC:
            return kSecAttrProtocolEPPC;
        case HKeyChainStoreProtocolTypeNNTPS:
            return kSecAttrProtocolNNTPS;
        case HKeyChainStoreProtocolTypeLDAPS:
            return kSecAttrProtocolLDAPS;
        case HKeyChainStoreProtocolTypeTelnetS:
            return kSecAttrProtocolTelnetS;
        case HKeyChainStoreProtocolTypeIRCS:
            return kSecAttrProtocolIRCS;
        case HKeyChainStoreProtocolTypePOP3S:
...
- (CFTypeRef)authenticationTypeObject {
    switch (_authenticationType) {
        case HKeyChainStoreAuthenticationTypeNTLM:
            return kSecAttrAuthenticationTypeNTLM;
        case HKeyChainStoreAuthenticationTypeMSN:
            return kSecAttrAuthenticationTypeMSN;
        case HKeyChainStoreAuthenticationTypeDPA:
            return kSecAttrAuthenticationTypeDPA;
        case HKeyChainStoreAuthenticationTypeRPA:
            return kSecAttrAuthenticationTypeRPA;
        case HKeyChainStoreAuthenticationTypeHTTPBasic:
            return kSecAttrAuthenticationTypeHTTPBasic;
        case HKeyChainStoreAuthenticationTypeHTTPDigest:
            return kSecAttrAuthenticationTypeHTTPDigest;
        case HKeyChainStoreAuthenticationTypeHTMLForm:
            return kSecAttrAuthenticationTypeHTMLForm;
        case HKeyChainStoreAuthenticationTypeDefault:
...
- (CFTypeRef)accessibilityObject {
    switch (_accessibility) {
        case HKeyChainStoreAccessibilityWhenUnlocked:
            return kSecAttrAccessibleWhenUnlocked;
        case HKeyChainStoreAccessibilityAfterFirstUnlock:
            return kSecAttrAccessibleAfterFirstUnlock;
        case HKeyChainStoreAccessibilityAlways:
            return kSecAttrAccessibleAlways;
        case HKeyChainStoreAccessibilityWhenPasscodeSetThisDeviceOnly:
            return kSecAttrAccessibleWhenPasscodeSetThisDeviceOnly;
        case HKeyChainStoreAccessibilityWhenUnlockedThisDeviceOnly:
            return kSecAttrAccessibleWhenUnlockedThisDeviceOnly;
        case HKeyChainStoreAccessibilityAfterFirstUnlockThisDeviceOnly:
            return kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly;
        case HKeyChainStoreAccessibilityAlwaysThisDeviceOnly:
...
+ (NSError *)argumentError:(NSString *)message {
    NSError *error = [NSError errorWithDomain:HKeyChainStoreErrorDomain code:HKeyChainStoreErrorInvalidArguments userInfo:@{NSLocalizedDescriptionKey: message}];
    NSLog(@"error: [%@] %@", @(error.code), error.localizedDescription);
    return error;
}

+ (NSError *)conversionError:(NSString *)message {
    NSError *error = [NSError errorWithDomain:HKeyChainStoreErrorDomain code:-67594 userInfo:@{NSLocalizedDescriptionKey: message}];
    NSLog(@"error: [%@] %@", @(error.code), error.localizedDescription);
    return error;
}
...
#endif
    NSError *error = [NSError errorWithDomain:HKeyChainStoreErrorDomain code:status userInfo:@{NSLocalizedDescriptionKey: message}];
    NSLog(@"OSStatus error: [%@] %@", @(error.code), error.localizedDescription);
    return error;
}

+ (NSError *)unexpectedError:(NSString *)message {
    NSError *error = [NSError errorWithDomain:HKeyChainStoreErrorDomain code:-99999 userInfo:@{NSLocalizedDescriptionKey: message}];
    NSLog(@"error: [%@] %@", @(error.code), error.localizedDescription);
    return error;
}
...
@implementation HKeyChainStore (Deprecation)
