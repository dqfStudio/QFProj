 ...
 */
static NSInteger const kHDefaultNumberOfPages = 0;

/**
 *  Default current page for initialization
 */
static NSInteger const kHDefaultCurrentPage = 0;

/**
 *  Default setting for hide for single page feature. For initialization
 */
static BOOL const kHDefaultHideForSinglePage = NO;

/**
 *  Default setting for shouldResizeFromCenter. For initialiation
 */
static BOOL const kHDefaultShouldResizeFromCenter = YES;

/**
 *  Default spacing between dots
 */
static NSInteger const kHDefaultSpacingBetweenDots = 8;

/**
 *  Default dot size
 */
static CGSize const kHDefaultDotSize = {8, 8};


@interface HPageControl()
 ...
 */
- (void)initialization {
    self.dotViewClass           = [HAnimatedDotView class];
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (touch.view != self) {
        ...
        if ([self.delegate respondsToSelector:@selector(HPageControl:didSelectPageAtIndex:)]) {
