...
#import "HRichLabelLayout.h"

@implementation HRichLabelLayout {
    CGRect _pathBox;
    CTFramesetterRef _frameSetter;
    CTFrameRef _frame;
    
    NSArray<HRichLabelLine *> *_lines;
    HTextInfoContainer *_infoContainer;
    
    CGPoint _point;
}

static void HTextTruncationAttributs(NSMutableDictionary *_Nullable* _Nonnull attrs) {
    HTextRemoveCustomAttributes(attrs);
...
+ (instancetype)layoutWithContainer:(HRichLabelContainer *)container atrributedString:(NSAttributedString *)text range:(NSRange)range {
    ...
    HRichLabelLayout *layout = [[self alloc] init];
...
- (void)_layoutWithContainer:(HRichLabelContainer *)container attributedString:(NSAttributedString *)text range:(NSRange)range {
...
- (void)_configFrame {
    ...
    CFRange range = HTextCFRangeFromNSRange(_range);
    ...
    boundingSize = HTextCeilSize(boundingSize);
...
- (void)_updateProperties {
    ...
    NSMutableArray<HRichLabelLine *> *lines = @[].mutableCopy;
    ...
    for (int i = 0; i < rows; i++) {
        ...
        HRichLabelLine *line = [HRichLabelLine lineWithCTLine:ctLine position:position point:_point];
        line.index = i;
...
- (HRichLabelLine *)_createTruncatedLineFromLine:(HRichLabelLine *)targetLine {
    ...
    if (runCount > 0) {
        ...
        if (cfAttrs) {
            ...
            HTextTruncationAttributs(&attrs);
        }
    }
    
    NSAttributedString *truncationToken = [[NSAttributedString alloc] initWithString:HTextTruncationToken attributes:attrs];
    ...
    HRichLabelLine *patchLine = [HRichLabelLine lineWithCTLine:truncatedLine position:position point:_point];
...
- (void)_drawTextInContext:(CGContextRef)context {
    ...
    CGContextSaveGState(context); {
        ...
        for (HRichLabelLine *line in _lines) {
...
- (void)_drawAttachmentsInContext:(CGContextRef)context targetLayer:(CALayer *)targetLayer targetView:(UIView *)targetView {
    ...
    [_infoContainer.rects enumerateObjectsUsingBlock:^(NSValue * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        HTextAttachment *attachment = self->_infoContainer.attachmentDict[key];
...
- (void)_drawBorderInContext:(CGContextRef)context {
    [_infoContainer.rects enumerateObjectsUsingBlock:^(NSValue * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        CGRect rect = obj.CGRectValue;
        NSString *key = NSStringFromCGRect(rect);
        HTextBorder *border = self->_infoContainer.borderDict[key];
...
- (void)_drawHighightInContext:(CGContextRef)context {
    [_infoContainer.rects enumerateObjectsUsingBlock:^(NSValue * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        CGRect rect = obj.CGRectValue;
        NSString *key = NSStringFromCGRect(rect);
        HTextHighlight *highlight = self->_infoContainer.highlightDict[key];
