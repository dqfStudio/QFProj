...
@interface HRichLabel() <HAsyncLayerDelegate> {
    NSMutableAttributedString *_innerText;
    HRichLabelLayout *_layout;
    HRichLabelLayout *_highlightLayout;
...
#pragma mark - HAsyncLayerDelegate

- (HAsyncLayerDisplayTask *)newAsyncLayerDisplayTask {
    
    HAsyncLayerDisplayTask *task = [HAsyncLayerDisplayTask new];
    task.willDisplay = ^(CALayer * _Nonnull layer) {
        [self _prepareDrawStringWithText];
        if (!self->_highlightLayout) [self _createLayout];
    ...
    task.display = ^(CGContextRef  _Nonnull context, CGSize size, BOOL (^ _Nonnull isCanceled)(void)) {
        if (isCanceled() || self->_innerText.length == 0) return;
        
        HRichLabelLayout *drawLayout = self->_highlightLayout ? _highlightLayout : _layout;
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    point = HTextRoundPoint(point);
    BOOL canResponse = [_layout.infoContainer canResponseUserActionAtPoint:point];
    HTextInfo *info = _layout.infoContainer.responseInfo;
...
- (HRichLabelContainer *)_createContainer {
    HRichLabelContainer *container = [HRichLabelContainer container];
...
- (void)_updateAttachments {
    [self _removeAttachments];
    HRichLabelLayout *layout = _highlightLayout ? _highlightLayout : _layout;
...
- (void)_showHighlightWithInfo:(HTextInfo *)info {
    HTextHighlight *highlight = info.highlight;
...
- (void)_createLayout {
    HRichLabelContainer *container = [self _createContainer];
    _layout = [HRichLabelLayout layoutWithContainer:container atrributedString:_innerText];
...
- (void)_createHighlightLayoutWithText:(NSAttributedString *)text {
    HRichLabelContainer *container = [self _createContainer];
    _highlightLayout = [HRichLabelLayout layoutWithContainer:container atrributedString:text];
