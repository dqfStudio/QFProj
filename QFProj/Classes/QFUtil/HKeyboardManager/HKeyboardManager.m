...
#import "UIViewController+HEndEdit.h"

static HKeyboardManager *hShareKB = nil;

@interface HKeyboardManager() <UIGestureRecognizerDelegate>

@end

@implementation HKeyboardManager {
...
+ (HKeyboardManager *)share {
    if(hShareKB == nil){
        hShareKB = [[HKeyboardManager alloc]init];
    }
    return hShareKB;
}
...
- (void)h_startToolBar {
        ...
        if ([_textEditView isKindOfClass:[UITextView class]] && _textEditView.inputAccessoryView == nil) {
            [UIView animateWithDuration:0.00001 delay:0 options:0 animations:^{
                [self h_addToolbarIfRequired];
            } completion:^(BOOL finished) {
                [self.textEditView reloadInputViews];
            }];
        }
        else {
            [self h_addToolbarIfRequired];
...
- (void)textEditViewDidBeginEditing:(NSNotification*)notification {
    ...
    if (!_superScrollView) {
        if (superScrollView && (superScrollView.isScrollEnabled == YES)) {
            ...
            scrollVs = [superScrollView h_findFatherViewByClass:[UIScrollView class] mutableArray:scrollVs isMember:NO];
            if (scrollVs.count > 1) {
    ...
    if (_kbShow == NO) {
        _textEditViewRootVC = [_textEditView h_topMostController];
        if (_textEditViewRootVC == nil)  _textEditViewRootVC = [UIWindow h_topMostControllerForStack];
        
        _textEditViewRootVCOriginalFrame = _textEditViewRootVC.view.frame;
    }
    
    _textEditViewOriginalFrameForWindow = [[_textEditView superview] convertRect:_textEditViewOriginalFrame toView:[UIWindow h_getWindow]];
...
- (void)keyboardWillShow:(NSNotification*)aNotification {
    ...
    if (_kbDuration&&_textEditView) {
        ...
        CGFloat move = 0 ;
        move = CGRectGetMaxY(_textEditViewOriginalFrameForWindow)-  (CGRectGetHeight([UIWindow h_getWindow].frame)-kbSize.height);
        _move = move;
        ...
        if (_superScrollView == nil) {
            if (_move > 0) {
                ...
                [_textEditViewRootVC hkbEndEdit];
