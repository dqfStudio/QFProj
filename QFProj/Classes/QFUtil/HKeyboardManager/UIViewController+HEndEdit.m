...
@implementation UIViewController (HEndEdit)
- (void)hkbEndEdit {
    SEL selectors[] = {
    ...
    for (NSUInteger index = 0; index < sizeof(selectors) / sizeof(SEL); ++index) {
        ...
        SEL swizzledSelector = NSSelectorFromString([@"hkb" stringByAppendingString:NSStringFromSelector(originalSelector)]);
...
- (void)hkbviewWillDisappear:(BOOL)animated {
    [self.view endEditing:YES];
    [self hkbviewWillDisappear:animated];
}
@end
