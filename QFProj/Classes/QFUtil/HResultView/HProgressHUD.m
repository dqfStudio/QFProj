...
@property (atomic, H_STRONG) UIView *indicator;
@property (atomic, H_STRONG) NSTimer *graceTimer;
@property (atomic, H_STRONG) NSTimer *minShowTimer;
@property (atomic, H_STRONG) NSDate *showStarted;
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	HProgressHUD *hud = [self HUDForView:view];
	if (hud != nil) {
...
+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
	NSArray *huds = [HProgressHUD allHUDsForView:view];
	for (HProgressHUD *hud in huds) {
		hud.removeFromSuperViewOnHide = YES;
		[hud hide:animated];
	}
	return [huds count];
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
		...
		self.animationType = HProgressHUDAnimationFade;
		self.mode = HProgressHUDModeIndeterminate;
		self.labelText = nil;
...
- (void)showUsingAnimation:(BOOL)animated {
	if (animated && animationType == HProgressHUDAnimationZoomIn) {
		self.transform = CGAffineTransformConcat(rotationTransform, CGAffineTransformMakeScale(0.5f, 0.5f));
	} else if (animated && animationType == HProgressHUDAnimationZoomOut) {
	...
	if (animated) {
		...
		self.alpha = 1.0f;
		if (animationType == HProgressHUDAnimationZoomIn || animationType == HProgressHUDAnimationZoomOut) {
...
- (void)hideUsingAnimation:(BOOL)animated {
	...
	if (animated && showStarted) {
		...
		// in the done method
		if (animationType == HProgressHUDAnimationZoomIn) {
			self.transform = CGAffineTransformConcat(rotationTransform, CGAffineTransformMakeScale(1.5f, 1.5f));
		} else if (animationType == HProgressHUDAnimationZoomOut) {
			self.transform = CGAffineTransformConcat(rotationTransform, CGAffineTransformMakeScale(0.5f, 0.5f));
		}
...
- (void)updateIndicators {
	...
	if (mode == HProgressHUDModeIndeterminate &&  !isActivityIndicator) {
	...
	else if (mode == HProgressHUDModeDeterminateHorizontalBar) {
		// Update to bar determinate indicator
		[indicator removeFromSuperview];
        self.indicator = MB_AUTORELEASE([[MBBarProgressView alloc] init]);
		[self addSubview:indicator];
	}
	else if (mode == HProgressHUDModeDeterminate || mode == HProgressHUDModeAnnularDeterminate) {
		if (!isRoundIndicator) {
		...
		if (mode == HProgressHUDModeAnnularDeterminate) {
			[(MBRoundProgressView *)indicator setAnnular:YES];
		}
	} 
	else if (mode == HProgressHUDModeCustomView && customView != indicator) {
		// Update custom view indicator
		[indicator removeFromSuperview];
		self.indicator = customView;
		[self addSubview:indicator];
	} else if (mode == HProgressHUDModeText) {
		[indicator removeFromSuperview];
		self.indicator = nil;
	}
}
