...
#import "HDrawerViewController.h"
...
@interface HDrawerViewController () <UIGestureRecognizerDelegate>
...
@property (nonatomic, strong) UIControl *rightblackMaskView;
@property (nonatomic, strong) NSHashTable<id<HDrawerViewControllerDelegate>> *delegates;
@end

@implementation HDrawerViewController
...
- (instancetype)initWithMainViewController:(UIViewController *)mainViewController
    ...
    if (self) {
        ...
        _visibleMenuWidth = AdaptW(kDefaultVisibleMenuWidth);
        _canDragMenu = YES;
...
- (void)bind:(id<HDrawerViewControllerDelegate>)delegate {
    if (!delegate) return;
    if (![delegate conformsToProtocol:@protocol(HDrawerViewControllerDelegate)]) return;
...
- (void)sendMenuDidAppearNotification {
    for (id<HDrawerViewControllerDelegate> delegate in [self.delegates setRepresentation]) {
        if (delegate && [delegate respondsToSelector:@selector(menuDidAppear)]) {
            [delegate menuDidAppear];
        }
    }
    if (![self.mainViewController.view.subviews containsObject:self.rightblackMaskView]) {
        [self.mainViewController.view addSubview:self.rightblackMaskView];
        [self.mainViewController.view bringSubviewToFront:self.rightblackMaskView];
    }
}
...
- (void)sendMenuDidDisappearNotification {
    ...
    if ([self.mainViewController.view.subviews containsObject:self.rightblackMaskView]) {
        [self.rightblackMaskView removeFromSuperview];
    }
}
...
- (void)tapGestureReceive:(UITapGestureRecognizer *)sender {
...
- (UIControl *)rightblackMaskView {
    if (!_rightblackMaskView) {
        _rightblackMaskView = [[UIControl alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.mainViewController.view.bounds), CGRectGetHeight(self.mainViewController.view.bounds))];
        _rightblackMaskView.backgroundColor = [UIColor clearColor];
        [_rightblackMaskView addTarget:self action:@selector(rightblackMaskViewAction) forControlEvents:UIControlEventTouchUpInside];
    }
    return _rightblackMaskView;
}

- (void)rightblackMaskViewAction {
    [self dismissMenuViewController];
}

- (NSHashTable<id<HDrawerViewControllerDelegate>> *)delegates {
    if (!_delegates) {
