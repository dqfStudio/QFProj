...
#pragma mark - 私有实现KVO的真实target类
#pragma mark - 每一个target对应了一个keyPath和监听该keyPath的所有block，
...
- (void)h_addObserverBlockForKeyPath:(NSString *)keyPath block:(void (^)(id obj, id oldVal, id newVal))block {
    ...
    //取出存有所有KVOTarget的字典
    NSMutableDictionary *allTargets = objc_getAssociatedObject(self, HKVOBlockKey);
    if (!allTargets) {
        //没有则创建
        allTargets = [NSMutableDictionary new];
        //绑定在该对象中
        objc_setAssociatedObject(self, HKVOBlockKey, allTargets, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    //获取对应keyPath中的所有target
    _HBlockTarget *targetForKeyPath = allTargets[keyPath];
    if (!targetForKeyPath) {
        //没有则创建
        targetForKeyPath = [_HBlockTarget new];
        //保存
    ...
    [targetForKeyPath h_addBlock:block];
    //对第一次注册KVO的类进行dealloc方法调剂
    [self _h_swizzleDealloc];
    dispatch_semaphore_signal(kvoSemaphore);
}

- (void)h_removeObserverBlockForKeyPath:(NSString *)keyPath {
    if (!keyPath.length) return;
    NSMutableDictionary *allTargets = objc_getAssociatedObject(self, HKVOBlockKey);
    if (!allTargets) return;
    _HBlockTarget *target = allTargets[keyPath];
    if (!target) return;
    dispatch_semaphore_t kvoSemaphore = [self _h_getSemaphoreWithKey:HKVOSemaphoreKey];
    dispatch_semaphore_wait(kvoSemaphore, DISPATCH_TIME_FOREVER);
...
- (void)h_removeAllObserverBlocks {
    NSMutableDictionary *allTargets = objc_getAssociatedObject(self, HKVOBlockKey);
    if (!allTargets) return;
    dispatch_semaphore_t kvoSemaphore = [self _h_getSemaphoreWithKey:HKVOSemaphoreKey];
    dispatch_semaphore_wait(kvoSemaphore, DISPATCH_TIME_FOREVER);
    [allTargets enumerateKeysAndObjectsUsingBlock:^(id key, _HBlockTarget *target, BOOL *stop) {
...
- (void)h_addNotificationForName:(NSString *)name block:(void (^)(NSNotification *notify))block {
    ...
    if (!target) {
        target = [_HBlockTarget new];
        allTargets[name] = target;
        [[NSNotificationCenter defaultCenter] addObserver:target selector:@selector(h_doNotification:) name:name object:nil];
    }
    [target h_addNotificationBlock:block];
    [self _h_swizzleDealloc];
...
- (void)_h_swizzleDealloc {
    ...
    @synchronized(swizzleClass) {
        ...
        id newDealloc = ^(__unsafe_unretained id objSelf){
            //在这里我们移除所有的KVO
            [objSelf h_removeAllObserverBlocks];
            //移除所有通知
            [objSelf h_removeAllNotification];
...
- (dispatch_semaphore_t)_h_getSemaphoreWithKey:(void *)key {
