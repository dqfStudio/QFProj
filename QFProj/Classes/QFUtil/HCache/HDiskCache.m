...
- (instancetype)initWithName:(NSString *)name {
    NSString *directoryPath = [[HDiskCache cachesDirectoryPath] stringByAppendingPathComponent:name];
    return [self initWithPath:directoryPath error:nil];
}

- (void)cleanup {
    if (_capacity == HDiskCacheCapacityUnlimited) {
        return;
    }
