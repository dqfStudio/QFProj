...
/*! Extended attribute name used to store metadata (see NSURL+HExtendedFileAttributes).
 */
extern NSString *const HCacheAttributeMetadataKey;
...
/*! Asynchronous composite in-memory and on-disk cache with LRU cleanup.
 @discussion Uses NSCache for in-memory caching and HDiskCache for on-disk caching. Provides API for associating metadata with cache entries.
 @note Encoding and decoding is implemented using id<HValueTransforming> protocol. HCache has several builtin value transformers that support object conforming to <NSCoding> protocol and images (UIImage). Use value transformer factory (id<HValueTransformerFactory>) to extend cache functionality.
 @note All disk IO operations (including operations that associate metadata with cache entries) are run on a serial dispatch queue. If you store the object using HCache asynchronous API and then immediately retrieve it you are guaranteed to get the object back.
 ...
 */
- (instancetype)initWithDiskCache:(nullable HDiskCache *)diskCache memoryCache:(nullable NSCache *)memoryCache NS_DESIGNATED_INITIALIZER;

/*! Initializes cache by creating HDiskCache instance with a given name and calling designated initializer.
...
/*! Initializes cache by creating HDiskCache instance with a given name and NSCache instance and calling designated initializer.
 @param name Name used to initialize disk cache. Raises NSInvalidArgumentException if name length is 0.
 */
...
/*! The transformer factory used by cache. Cache is initialized with a default value transformer factory. For more info see HValueTransformerFactory declaration.
 */
@property (nonatomic) id<HValueTransformerFactory> valueTransfomerFactory;

/*! Returns disk cache used by receiver.
 */
@property (nullable, nonatomic, readonly) HDiskCache *diskCache;
...
@interface HCache (HCacheExtended)
