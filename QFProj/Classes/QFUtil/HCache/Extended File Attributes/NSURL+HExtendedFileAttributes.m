...
@implementation NSURL (HExtendedFileAttributes)

- (int)h_setExtendedAttributeValue:(id<NSCoding>)value forKey:(NSString *)key {
    NSData *data = [NSKeyedArchiver archivedDataWithRootObject:value];
    return [self h_setExtendedAttributeData:data forKey:key options:0];
}

- (int)h_setExtendedAttributeData:(NSData *)data forKey:(NSString *)key options:(int)options {
...
- (id)h_extendedAttributeValueForKey:(NSString *)key error:(int *)error {
    NSData *data = [self h_extendedAttributeDataForKey:key error:error options:0];
    return key ? [NSKeyedUnarchiver unarchiveObjectWithData:data] : nil;
}

- (NSData *)h_extendedAttributeDataForKey:(NSString *)k error:(int *)error options:(int)options {
...
- (int)h_removeExtendedAttributeForKey:(NSString *)key {
    return [self h_removeExtendedAttributeForKey:key options:0];
}

- (int)h_removeExtendedAttributeForKey:(NSString *)key options:(int)options {
    int error = removexattr(self.path.fileSystemRepresentation, [key UTF8String], options);
    return !error ? 0 : errno;
}

- (NSArray *)h_extendedAttributesList:(int *)error {
    return [self h_extendedAttributesList:error options:0];
}

- (NSArray *)h_extendedAttributesList:(int *)error options:(int)options {
    const char *path = self.path.fileSystemRepresentation;
    char *buffer;
