...
#define HTEXT_SWAP(_a_, _b_)  do { __typeof__(_a_) _tmp_ = (_a_); (_a_) = (_b_); (_b_) = _tmp_; } while (0)
#endif
...
/**
 ...
 @return YES or NO.
 */
static inline BOOL HTextIsLinebreakChar(unichar c) {
    switch (c) {
        case 0x000D:
        case 0x2028:
        case 0x000A:
 ...
 */
static inline NSTextCheckingType HTextNSTextCheckingTypeFromUIDataDetectorType(UIDataDetectorTypes types) {
 ...
 */
static inline BOOL HTextUIFontIsEmoji(UIFont *font) {
...
/**
 ...
 @return YES: the font is Emoji, NO: the font is not Emoji.
 */
static inline BOOL HTextCTFontIsEmoji(CTFontRef font) {
    BOOL isEmoji = NO;
...
/**
 ...
 @return YES: the font is Emoji, NO: the font is not Emoji.
 */
static inline BOOL HTextCGFontIsEmoji(CGFontRef font) {
    BOOL isEmoji = NO;
 ...
 */
static inline BOOL HTextCTFontContainsColorBitmapGlyphs(CTFontRef font) {
 ...
 */
static inline BOOL HTextCGGlyphIsBitmap(CTFontRef font, CGGlyph glyph) {
    if (!font && !glyph) return NO;
    if (!HTextCTFontContainsColorBitmapGlyphs(font)) return NO;
 ...
 */
static inline CGFloat HTextEmojiGetAscentWithFontSize(CGFloat fontSize) {
    if (fontSize < 16) {
 ...
 */
static inline CGFloat HTextEmojiGetDescentWithFontSize(CGFloat fontSize) {
    if (fontSize < 16) {
 ...
 */
static inline CGRect HTextEmojiGetGlyphBoundingRectWithFontSize(CGFloat fontSize) {
    CGRect rect;
    rect.origin.x = 0.75;
    rect.size.width = rect.size.height = HTextEmojiGetAscentWithFontSize(fontSize);
    if (fontSize < 16) {
 ...
 */
NSCharacterSet *HTextVerticalFormRotateCharacterSet(void);

/**
 Get the character set which should rotate and move in vertical form.
 @return The shared character set.
 */
NSCharacterSet *HTextVerticalFormRotateAndMoveCharacterSet(void);
...
static inline CGFloat HTextCGAffineTransformGetRotation(CGAffineTransform transform) {
...
static inline CGFloat HTextCGAffineTransformGetTranslateX(CGAffineTransform transform) {
    return transform.tx;
}

/// Get the transform's translate.y
static inline CGFloat HTextCGAffineTransformGetTranslateY(CGAffineTransform transform) {
    return transform.ty;
}
 ...
 */
CGAffineTransform HTextCGAffineTransformGetFromPoints(CGPoint before[_Nullable 3], CGPoint after[_Nullable 3]);

/// Get the transform which can converts a point from the coordinate system of a given view to another.
CGAffineTransform HTextCGAffineTransformGetFromViews(UIView *from, UIView *to);

/// Create a skew transform.
static inline CGAffineTransform HTextCGAffineTransformMakeSkew(CGFloat x, CGFloat y){
...
static inline UIEdgeInsets HTextUIEdgeInsetsInvert(UIEdgeInsets insets) {
    return UIEdgeInsetsMake(-insets.top, -insets.left, -insets.bottom, -insets.right);
}

/// Convert CALayer's gravity string to UIViewContentMode.
UIViewContentMode HTextCAGravityToUIViewContentMode(NSString *gravity);

/// Convert UIViewContentMode to CALayer's gravity string.
NSString *HTextUIViewContentModeToCAGravity(UIViewContentMode contentMode);
 ...
 */
CGRect HTextCGRectFitWithContentMode(CGRect rect, CGSize size, UIViewContentMode mode);

/// Returns the center for the rectangle.
static inline CGPoint HTextCGRectGetCenter(CGRect rect) {
...
static inline CGFloat HTextCGPointGetDistanceToPoint(CGPoint p1, CGPoint p2) {
    return sqrt((p1.x - p2.x) * (p1.x - p2.x) + (p1.y - p2.y) * (p1.y - p2.y));
}

/// Returns the minmium distance between a point to a rectangle.
static inline CGFloat HTextCGPointGetDistanceToRect(CGPoint p, CGRect r) {
...
CGFloat HTextScreenScale(void);

/// Get main screen's size. Height is always larger than width.
CGSize HTextScreenSize(void);
...
static inline CGFloat HTextCGFloatFromPixel(CGFloat value) {
    return value / HTextScreenScale();
}
...
/// floor point value for pixel-aligned
static inline CGRect HTextCGRectPixelFloor(CGRect rect) {
    CGPoint origin = HTextCGPointPixelCeil(rect.origin);
    CGPoint corner = HTextCGPointPixelFloor(CGPointMake(rect.origin.x + rect.size.width,
...
/// round point value for pixel-aligned
static inline CGRect HTextCGRectPixelRound(CGRect rect) {
    CGPoint origin = HTextCGPointPixelRound(rect.origin);
    CGPoint corner = HTextCGPointPixelRound(CGPointMake(rect.origin.x + rect.size.width,
                                                     rect.origin.y + rect.size.height));
    return CGRectMake(origin.x, origin.y, corner.x - origin.x, corner.y - origin.y);
}

/// ceil point value for pixel-aligned
static inline CGRect HTextCGRectPixelCeil(CGRect rect) {
    CGPoint origin = HTextCGPointPixelFloor(rect.origin);
    CGPoint corner = HTextCGPointPixelCeil(CGPointMake(rect.origin.x + rect.size.width,
                                                    rect.origin.y + rect.size.height));
    return CGRectMake(origin.x, origin.y, corner.x - origin.x, corner.y - origin.y);
}

/// round point value to .5 pixel for path stroke (odd pixel line width pixel-aligned)
static inline CGRect HTextCGRectPixelHalf(CGRect rect) {
    CGPoint origin = HTextCGPointPixelHalf(rect.origin);
    CGPoint corner = HTextCGPointPixelHalf(CGPointMake(rect.origin.x + rect.size.width,
                                                    rect.origin.y + rect.size.height));
    return CGRectMake(origin.x, origin.y, corner.x - origin.x, corner.y - origin.y);
}
...
static inline UIEdgeInsets HTextUIEdgeInsetPixelFloor(UIEdgeInsets insets) {
    insets.top = HTextCGFloatPixelFloor(insets.top);
    insets.left = HTextCGFloatPixelFloor(insets.left);
    insets.bottom = HTextCGFloatPixelFloor(insets.bottom);
    insets.right = HTextCGFloatPixelFloor(insets.right);
    return insets;
}

/// ceil UIEdgeInset for pixel-aligned
static inline UIEdgeInsets HTextUIEdgeInsetPixelCeil(UIEdgeInsets insets) {
    insets.top = HTextCGFloatPixelCeil(insets.top);
    insets.left = HTextCGFloatPixelCeil(insets.left);
    insets.bottom = HTextCGFloatPixelCeil(insets.bottom);
    insets.right = HTextCGFloatPixelCeil(insets.right);
    return insets;
}
