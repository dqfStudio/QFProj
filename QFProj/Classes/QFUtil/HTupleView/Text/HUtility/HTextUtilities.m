...
NSCharacterSet *HTextVerticalFormRotateCharacterSet() {
...
NSCharacterSet *HTextVerticalFormRotateAndMoveCharacterSet() {
    static NSMutableCharacterSet *set;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        set = [NSMutableCharacterSet new];
...
CGAffineTransform HTextCGAffineTransformGetFromPoints(CGPoint before[3], CGPoint after[3]) {
...
CGAffineTransform HTextCGAffineTransformGetFromViews(UIView *from, UIView *to) {
    ...
    after[0] = [from h_convertPoint:before[0] toViewOrWindow:to];
    after[1] = [from h_convertPoint:before[1] toViewOrWindow:to];
    after[2] = [from h_convertPoint:before[2] toViewOrWindow:to];
    
    return HTextCGAffineTransformGetFromPoints(before, after);
}
...
UIApplication *HTextSharedApplication() {
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wundeclared-selector"
    return HTextIsAppExtension() ? nil : [UIApplication performSelector:@selector(sharedApplication)];
