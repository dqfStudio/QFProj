...
@implementation HTextEffectWindow

+ (instancetype)sharedWindow {
    static HTextEffectWindow *one = nil;
    if (one == nil) {
        // iOS 9 compatible
    ...
    dispatch_once(&onceToken, ^{
        if (!HTextIsAppExtension()) {
            one = [self new];
            one.frame = (CGRect){.size = HTextScreenSize()};
...
- (void)becomeKeyWindow {
    [[HTextSharedApplication().delegate window] makeKeyWindow];
}

- (UIViewController *)rootViewController {
    for (UIWindow *window in [HTextSharedApplication() windows]) {
...
- (HTextDirection)_keyboardDirection {
...
- (CGPoint)_correctedCaptureCenter:(CGPoint)center {
    ...
    if (!CGRectIsNull(keyboardFrame) && !CGRectIsEmpty(keyboardFrame)) {
        HTextDirection direction = [self _keyboardDirection];
        switch (direction) {
...
- (CGPoint)_correctedCenter:(CGPoint)center forMagnifier:(HTextMagnifier *)mag rotation:(CGFloat)rotation {
    CGFloat degree = HTextRadiansToDegrees(rotation);
 ...
 */
- (CGFloat)_updateMagnifier:(HTextMagnifier *)mag {
    UIApplication *app = HTextSharedApplication();
    if (!app) return 0;
    ...
    CGPoint captureCenter = [self h_convertPoint:mag.hostCaptureCenter fromViewOrWindow:hostView];
    ...
    CGAffineTransform trans = HTextCGAffineTransformGetFromViews(hostView, self);
    CGFloat rotation = HTextCGAffineTransformGetRotation(trans);
    ...
    for (UIWindow *window in windows) {
        ...
        CGContextConcatCTM(context, HTextCGAffineTransformGetFromViews(window, self));
...
- (void)showMagnifier:(HTextMagnifier *)mag {
    ...
    CGPoint center = [self h_convertPoint:mag.hostPopoverCenter fromViewOrWindow:mag.hostView];
    CGAffineTransform trans = CGAffineTransformMakeRotation(rotation);
    trans = CGAffineTransformScale(trans, 0.3, 0.3);
    mag.transform = trans;
    mag.center = center;
    if (mag.type == HTextMagnifierTypeRanged) {
        mag.alpha = 0;
    }
    NSTimeInterval time = mag.type == HTextMagnifierTypeCaret ? 0.08 : 0.1;
...
- (void)hideMagnifier:(HTextMagnifier *)mag {
    ...
    CGFloat rotation = [self _updateMagnifier:mag];
    CGPoint center = [self h_convertPoint:mag.hostPopoverCenter fromViewOrWindow:mag.hostView];
    NSTimeInterval time = mag.type == HTextMagnifierTypeCaret ? 0.20 : 0.15;
    [UIView animateWithDuration:time delay:0 options:UIViewAnimationOptionCurveEaseInOut | UIViewAnimationOptionAllowUserInteraction | UIViewAnimationOptionBeginFromCurrentState animations:^{
...
- (void)_updateSelectionGrabberDot:(HSelectionGrabberDot *)dot selection:(HTextSelectionView *)selection {
    ...
    if (selection.hostView.clipsToBounds == YES && dot.h_visibleAlpha > 0.1) {
        ...
        if (!dotInKeyboard) {
            ...
            if (HTextCGRectGetArea(intersection) < HTextCGRectGetArea(dotRect)) {
                CGFloat dist = HTextCGPointGetDistanceToRect(HTextCGRectGetCenter(dotRect), hostRect);
