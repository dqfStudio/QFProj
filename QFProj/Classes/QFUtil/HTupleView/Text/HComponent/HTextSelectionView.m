...
@implementation HSelectionGrabberDot
...
- (void)setDotDirection:(HTextDirection)dotDirection {
    _dotDirection = dotDirection;
    [self addSubview:_dot];
    CGRect frame = _dot.frame;
    CGFloat ofs = 0.5;
    if (dotDirection == HTextDirectionTop) {
        frame.origin.y = -frame.size.height + ofs;
        frame.origin.x = (self.bounds.size.width - frame.size.width) / 2;
    } else if (dotDirection == HTextDirectionRight) {
        frame.origin.x = self.bounds.size.width - ofs;
        frame.origin.y = (self.bounds.size.height - frame.size.height) / 2;
    } else if (dotDirection == HTextDirectionBottom) {
...
- (CGRect)touchRect {
    ...
    if (_dotDirection == HTextDirectionTop) {
        insets.top = -kTouchDotExtend;
    } else if (_dotDirection == HTextDirectionRight) {
        insets.right = -kTouchDotExtend;
    } else if (_dotDirection == HTextDirectionBottom) {
        insets.bottom = -kTouchDotExtend;
    } else if (_dotDirection == HTextDirectionLeft) {
...
@interface HTextSelectionView ()
@property (nonatomic, strong) NSTimer *caretTimer;
@property (nonatomic, strong) UIView *caretView;
@property (nonatomic, strong) HSelectionGrabber *startGrabber;
@property (nonatomic, strong) HSelectionGrabber *endGrabber;
@property (nonatomic, strong) NSMutableArray *markViews;
@end

@implementation HTextSelectionView
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    _startGrabber = [HSelectionGrabber new];
    _startGrabber.dotDirection = HTextDirectionTop;
    _startGrabber.hidden = YES;
    _endGrabber = [HSelectionGrabber new];
    _endGrabber.dotDirection = HTextDirectionBottom;
...
- (void)setVerticalForm:(BOOL)verticalForm {
    if (_verticalForm != verticalForm) {
        ...
        self.startGrabber.dotDirection = verticalForm ? HTextDirectionRight : HTextDirectionTop;
        self.endGrabber.dotDirection = verticalForm ? HTextDirectionLeft : HTextDirectionBottom;
    }
}
...
- (CGRect)_standardCaretRect:(CGRect)caretRect {
    ...
    caretRect = HTextCGRectPixelRound(caretRect);
...
- (void)setSelectionRects:(NSArray *)selectionRects {
    ...
    [selectionRects enumerateObjectsUsingBlock: ^(HTextSelectionRect *r, NSUInteger idx, BOOL *stop) {
        CGRect rect = r.rect;
...
- (BOOL)isStartGrabberContainsPoint:(CGPoint)point {
    ...
    if (CGRectIntersectsRect(startRect, endRect)) {
        CGFloat distStart = HTextCGPointGetDistanceToPoint(point, HTextCGRectGetCenter(startRect));
        CGFloat distEnd = HTextCGPointGetDistanceToPoint(point, HTextCGRectGetCenter(endRect));
...
- (BOOL)isSelectionRectsContainsPoint:(CGPoint)point {
    ...
    for (HTextSelectionRect *rect in _selectionRects) {
