...
- (void)reloadBounds {
    ...
    for (NSUInteger r = 0; r < runCount; r++) {
        ...
        HTextAttachment *attachment = attrs[HTextAttachmentAttributeName];
        if (attachment) {
            ...
            NSRange runRange = HTextNSRangeFromCFRange(CTRunGetStringRange(run));
...
- (NSString *)description {
    ...
    [desc appendFormat:@"<HTextLine: %p> row:%zd range:%tu,%tu",self, self.row, range.location, range.length];
