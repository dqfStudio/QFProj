...
@class HTextRunGlyphRange;

NS_ASSUME_NONNULL_BEGIN

/**
 A text line object wrapped `CTLineRef`, see `HTextLayout` for more.
 */
@interface HTextLine : NSObject

+ (instancetype)lineWithCTLine:(CTLineRef)CTLine position:(CGPoint)position vertical:(BOOL)isVertical;

@property (nonatomic) NSUInteger index;     ///< line index
@property (nonatomic) NSUInteger row;       ///< line row
@property (nullable, nonatomic, strong) NSArray<NSArray<HTextRunGlyphRange *> *> *verticalRotateRange; ///< Run rotate range
...
@property (nullable, nonatomic, readonly) NSArray<HTextAttachment *> *attachments; ///< HTextAttachment
...
typedef NS_ENUM(NSUInteger, HTextRunGlyphDrawMode) {
    /// No rotate.
    HTextRunGlyphDrawModeHorizontal = 0,
    
    /// Rotate vertical for single glyph.
    HTextRunGlyphDrawModeVerticalRotate = 1,
    
    /// Rotate vertical for single glyph, and move the glyph to a better position,
    /// such as fullwidth punctuation.
    HTextRunGlyphDrawModeVerticalRotateMove = 2,
};

/**
 A range in CTRun, used for vertical form.
 */
@interface HTextRunGlyphRange : NSObject
@property (nonatomic) NSRange glyphRangeInRun;
@property (nonatomic) HTextRunGlyphDrawMode drawMode;
+ (instancetype)rangeWithRange:(NSRange)range drawMode:(HTextRunGlyphDrawMode)mode;
@end
