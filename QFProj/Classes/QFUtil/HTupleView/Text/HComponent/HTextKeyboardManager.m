...
static int _HTextKeyboardViewFrameObserverKey;

/// Observer for view's frame/bounds/center/transform
@interface _HTextKeyboardViewFrameObserver : NSObject
...
@implementation _HTextKeyboardViewFrameObserver {
...
- (void)addToKeyboardView:(UIView *)keyboardView {
    ...
    if (_keyboardView) {
        ...
        objc_setAssociatedObject(_keyboardView, &_HTextKeyboardViewFrameObserverKey, nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    _keyboardView = keyboardView;
    if (keyboardView) {
        [self addFrameObserver];
    }
    objc_setAssociatedObject(keyboardView, &_HTextKeyboardViewFrameObserverKey, self, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
+ (instancetype)observerForView:(UIView *)keyboardView {
    ...
    return objc_getAssociatedObject(keyboardView, &_HTextKeyboardViewFrameObserverKey);
}
...
@implementation HTextKeyboardManager {
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"HTextKeyboardManager init error" reason:@"Use 'defaultManager' to get instance." userInfo:nil];
    return [super init];
}
...
- (void)_initFrameObserver {
    ...
    _HTextKeyboardViewFrameObserver *observer = [_HTextKeyboardViewFrameObserver observerForView:keyboardView];
    if (!observer) {
        observer = [_HTextKeyboardViewFrameObserver new];
...
+ (instancetype)defaultManager {
    static HTextKeyboardManager *mgr = nil;
...
- (void)addObserver:(id<HTextKeyboardObserver>)observer {
    if (!observer) return;
    [_observers addObject:observer];
}

- (void)removeObserver:(id<HTextKeyboardObserver>)observer {
    if (!observer) return;
...
- (void)_notifyAllObservers {
    ...
    HTextKeyboardTransition trans = {0};
    ...
    if (!CGRectEqualToRect(trans.toFrame, _fromFrame)) {
        for (id<HTextKeyboardObserver> observer in _observers.copy) {
