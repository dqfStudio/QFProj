...
const CGSize HTextContainerMaxSize = (CGSize){0x100000, 0x100000};
...
static inline CGSize HTextClipCGSize(CGSize size) {
    if (size.width > HTextContainerMaxSize.width) size.width = HTextContainerMaxSize.width;
    if (size.height > HTextContainerMaxSize.height) size.height = HTextContainerMaxSize.height;
    return size;
}
 ...
 */
static CGColorRef HTextGetCGColor(CGColorRef color) {
...
@implementation HTextLinePositionSimpleModifier
- (void)modifyLines:(NSArray *)lines fromText:(NSAttributedString *)text inContainer:(HTextContainer *)container {
    if (container.verticalForm) {
        for (NSUInteger i = 0, max = lines.count; i < max; i++) {
            HTextLine *line = lines[i];
...
- (id)copyWithZone:(NSZone *)zone {
    HTextLinePositionSimpleModifier *one = [self.class new];
...
@implementation HTextContainer {
    @package
    BOOL _readonly; ///< used only in HTextLayout.implementation
    ...
    HTextTruncationType _truncationType;
    NSAttributedString *_truncationToken;
    id<HTextLinePositionModifier> _linePositionModifier;
}
...
+ (instancetype)containerWithSize:(CGSize)size insets:(UIEdgeInsets)insets {
    HTextContainer *one = [self new];
    one.size = HTextClipCGSize(size);
    one.insets = insets;
    return one;
}
...
- (id)copyWithZone:(NSZone *)zone {
    HTextContainer *one = [self.class new];
...
@interface HTextLayout ()

@property (nonatomic, readwrite) HTextContainer *container;
...
@property (nonatomic, readwrite) HTextLine *truncatedLine;
...
@property (nonatomic, assign) HRowEdge *lineRowsEdge; ///< top-left origin

@end



@implementation HTextLayout
...
+ (HTextLayout *)layoutWithContainerSize:(CGSize)size text:(NSAttributedString *)text {
    HTextContainer *container = [HTextContainer containerWithSize:size];
    return [self layoutWithContainer:container text:text];
}

+ (HTextLayout *)layoutWithContainer:(HTextContainer *)container text:(NSAttributedString *)text {
    return [self layoutWithContainer:container text:text range:NSMakeRange(0, text.length)];
}

+ (HTextLayout *)layoutWithContainer:(HTextContainer *)container text:(NSAttributedString *)text range:(NSRange)range {
    HTextLayout *layout = NULL;
    ...
    HTextLine *truncatedLine = nil;
    HRowEdge *lineRowsEdge = NULL;
    ...
    layout = [[HTextLayout alloc] _init];
    layout.text = text;
    ...
    if (container.path == nil && container.exclusionPaths.count == 0) {
        ...
        if (needFixLayoutSizeBug) {
            ...
            if (container.isVerticalForm) {
                rect.size.width = HTextContainerMaxSize.width;
            }else {
                rect.size.height = HTextContainerMaxSize.height;
            }
        }
    ...
    ctFrame = CTFramesetterCreateFrame(ctSetter, HTextCFRangeFromNSRange(range), cgPath, (CFTypeRef)frameAttrs);
    ...
    for (NSUInteger i = 0; i < lineCount; i++) {
        ...
        HTextLine *line = [HTextLine lineWithCTLine:ctLine position:position vertical:isVerticalForm];
    ...
    if (rowCount > 0) {
        if (maximumNumberOfRows > 0) {
            if (rowCount > maximumNumberOfRows) {
                ...
                do {
                    HTextLine *line = lines.lastObject;
                    if (!line) break;
        ...
        HTextLine *lastLine = lines.lastObject;
        ...
        lineRowsEdge = calloc(rowCount, sizeof(HRowEdge));
        ...
        for (NSUInteger i = 0, max = lines.count; i < max; i++) {
            ...
            if ((NSInteger)line.row != lastRowIdx) {
                if (lastRowIdx >= 0) {
                    lineRowsEdge[lastRowIdx] = (HRowEdge) {.head = lastHead, .foot = lastFoot };
                }
                lastRowIdx = line.row;
        ...
        for (NSUInteger i = 1; i < rowCount; i++) {
            HRowEdge v0 = lineRowsEdge[i - 1];
            HRowEdge v1 = lineRowsEdge[i];
    ...
    { // calculate bounding size
        ...
        if (container.path) {
            ...
            rect = UIEdgeInsetsInsetRect(rect,HTextUIEdgeInsetsInvert(container.insets));
        }
        rect = CGRectStandardize(rect);
    ...
    visibleRange = HTextNSRangeFromCFRange(CTFrameGetVisibleStringRange(ctFrame));
    if (needTruncation) {
        HTextLine *lastLine = lines.lastObject;
        ...
        if (container.truncationType != HTextTruncationTypeNone) {
            ...
            if (container.truncationToken) {
                ...
                truncationToken = [[NSAttributedString alloc] initWithString:HTextTruncationToken attributes:attrs];
                truncationTokenLine = CTLineCreateWithAttributedString((CFAttributedStringRef)truncationToken);
            }
            if (truncationTokenLine) {
                CTLineTruncationType type = kCTLineTruncationEnd;
                if (container.truncationType == HTextTruncationTypeStart) {
                    type = kCTLineTruncationStart;
                } else if (container.truncationType == HTextTruncationTypeMiddle) {
                ...
                if (ctLastLineExtend) {
                    ...
                    if (ctTruncatedLine) {
                        truncatedLine = [HTextLine lineWithCTLine:ctTruncatedLine position:lastLine.position vertical:isVerticalForm];
    ...
    if (isVerticalForm) {
        NSCharacterSet *rotateCharset = HTextVerticalFormRotateCharacterSet();
        NSCharacterSet *rotateMoveCharset = HTextVerticalFormRotateAndMoveCharacterSet();
        
        void (^lineBlock)(HTextLine *) = ^(HTextLine *line){
            ...
            for (NSUInteger r = 0; r < runCount; r++) {
                ...
                BOOL isColorGlyph = HTextCTFontContainsColorBitmapGlyphs(font);
                
                NSUInteger prevIdx = 0;
                HTextRunGlyphDrawMode prevMode = HTextRunGlyphDrawModeHorizontal;
                ...
                for (NSUInteger g = 0; g < glyphCount; g++) {
                    ...
                    HTextRunGlyphDrawMode mode = glyphRotateMove ? HTextRunGlyphDrawModeVerticalRotateMove:(glyphRotate ? HTextRunGlyphDrawModeVerticalRotate : HTextRunGlyphDrawModeHorizontal);
                    if (g == 0) {
                        prevMode = mode;
                    } else if (mode != prevMode) {
                        HTextRunGlyphRange *aRange = [HTextRunGlyphRange rangeWithRange:NSMakeRange(prevIdx, g - prevIdx) drawMode:prevMode];
                ...
                if (prevIdx < glyphCount) {
                    HTextRunGlyphRange *aRange = [HTextRunGlyphRange rangeWithRange:NSMakeRange(prevIdx, glyphCount - prevIdx) drawMode:prevMode];
                    [runRanges addObject:aRange];
                }
                
            }
        };
        for (HTextLine *line in lines) {
            lineBlock(line);
        }
    ...
    if (visibleRange.length > 0) {
        ...
        void (^block)(NSDictionary *attrs, NSRange range, BOOL *stop) = ^(NSDictionary *attrs, NSRange range, BOOL *stop) {
            if (attrs[HTextHighlightAttributeName]) layout.containsHighlight = YES;
            if (attrs[HTextBlockBorderAttributeName]) layout.needDrawBlockBorder = YES;
            if (attrs[HTextBackgroundBorderAttributeName]) layout.needDrawBackgroundBorder = YES;
            if (attrs[HTextShadowAttributeName] || attrs[NSShadowAttributeName]) layout.needDrawShadow = YES;
            if (attrs[HTextUnderlineAttributeName]) layout.needDrawUnderline = YES;
            if (attrs[HTextAttachmentAttributeName]) layout.needDrawAttachment = YES;
            if (attrs[HTextInnerShadowAttributeName]) layout.needDrawInnerShadow = YES;
            if (attrs[HTextStrikethroughAttributeName]) layout.needDrawStrikethrough = YES;
            if (attrs[HTextBorderAttributeName]) layout.needDrawBorder = YES;
        };
    ...
    for (NSUInteger i = 0, max = lines.count; i < max; i++) {
        ...
        if (line.attachments.count > 0) {
            ...
            for (HTextAttachment *attachment in line.attachments) {
...
+ (NSArray *)layoutWithContainers:(NSArray *)containers text:(NSAttributedString *)text range:(NSRange)range {
    ...
    for (NSUInteger i = 0, max = containers.count; i < max; i++) {
        HTextContainer *container = containers[i];
        HTextLayout *layout = [self layoutWithContainer:container text:text range:range];
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    NSData *textData = [HTextArchiver archivedDataWithRootObject:_text];
...
- (id)initWithCoder:(NSCoder *)aDecoder {
    ...
    NSAttributedString *text = [HTextUnarchiver unarchiveObjectWithData:textData];
    HTextContainer *container = [aDecoder decodeObjectForKey:@"container"];
...
- (NSUInteger)_rowIndexForEdge:(CGFloat)edge {
    ...
    while (lo <= hi) {
        mid = (lo + hi) / 2;
        HRowEdge oneEdge = _lineRowsEdge[mid];
        if (isVertical ?
 ...
 */
- (CTRunRef)_runForLine:(HTextLine *)line position:(HTextPosition *)position {
    if (!line || !position) return NULL;
    CFArrayRef runs = CTLineGetGlyphRuns(line.CTLine);
    for (NSUInteger i = 0, max = CFArrayGetCount(runs); i < max; i++) {
        CTRunRef run = CFArrayGetValueAtIndex(runs, i);
        CFRange range = CTRunGetStringRange(run);
        if (position.affinity == HTextAffinityBackward) {
...
/**
 ...
 @param block    The block to be executed before returns YES.
            ...
            next:  right position
 */
- (BOOL)_insideComposedCharacterSequences:(HTextLine *)line position:(NSUInteger)position block:(void (^)(CGFloat left, CGFloat right, NSUInteger prev, NSUInteger next))block {
 ...
 */
- (BOOL)_insideEmoji:(HTextLine *)line position:(NSUInteger)position block:(void (^)(CGFloat left, CGFloat right, NSUInteger prev, NSUInteger next))block {
    if (!line) return NO;
    CFArrayRef runs = CTLineGetGlyphRuns(line.CTLine);
    for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
        ...
        if (!HTextCTFontContainsColorBitmapGlyphs(font)) continue;
...
/**
 ...
 @return YES if RTL.
 */
- (BOOL)_isRightToLeftInLine:(HTextLine *)line atPoint:(CGPoint)point {
    if (!line) return NO;
    // get write direction
    BOOL RTL = NO;
    CFArrayRef runs = CTLineGetGlyphRuns(line.CTLine);
 ...
 */
- (HTextRange *)_correctedRangeWithEdge:(HTextRange *)range {
    NSRange visibleRange = self.visibleRange;
    HTextPosition *start = range.start;
    HTextPosition *end = range.end;
    
    if (start.offset == visibleRange.location && start.affinity == HTextAffinityBackward) {
        start = [HTextPosition positionWithOffset:start.offset affinity:HTextAffinityForward];
    }
    
    if (end.offset == visibleRange.location + visibleRange.length && start.affinity == HTextAffinityForward) {
        end = [HTextPosition positionWithOffset:end.offset affinity:HTextAffinityBackward];
    }
    
    if (start != range.start || end != range.end) {
        range = [HTextRange rangeWithStart:start end:end];
    }
    return range;
}
...
- (NSUInteger)rowIndexForLine:(NSUInteger)line {
    ...
    return ((HTextLine *)_lines[line]).row;
}
...
- (NSUInteger)lineIndexForPoint:(CGPoint)point {
    ...
    for (NSUInteger i = lineIdx0; i <= lineIdx1; i++) {
        CGRect bounds = ((HTextLine *)_lines[i]).bounds;
...
- (CGFloat)offsetForTextPosition:(NSUInteger)position lineIndex:(NSUInteger)lineIndex {
    ...
    HTextLine *line = _lines[lineIndex];
...
- (NSUInteger)textPositionForPoint:(CGPoint)point lineIndex:(NSUInteger)lineIndex {
    ...
    for (NSUInteger r = 0, max = CFArrayGetCount(runs); r < max; r++) {
        ...
        if (range.location <= idx && idx < range.location + range.length) {
            ...
            CFDictionaryRef attrs = CTRunGetAttributes(run);
            CTFontRef font = CFDictionaryGetValue(attrs, kCTFontAttributeName);
            if (!HTextCTFontContainsColorBitmapGlyphs(font)) break;
...
- (HTextPosition *)closestPositionToPoint:(CGPoint)point {
    ...
    if (position <= _visibleRange.location) {
        return [HTextPosition positionWithOffset:_visibleRange.location affinity:HTextAffinityForward];
    } else if (position >= _visibleRange.location + _visibleRange.length) {
        return [HTextPosition positionWithOffset:_visibleRange.location + _visibleRange.length affinity:HTextAffinityBackward];
    }
    
    HTextAffinity finalAffinity = HTextAffinityForward;
    BOOL finalAffinityDetected = NO;
    
    // binding range
    NSRange bindingRange;
    HTextBinding *binding = [_text attribute:HTextBindingAttributeName atIndex:position longestEffectiveRange:&bindingRange inRange:NSMakeRange(0, _text.length)];
    if (binding && bindingRange.length > 0) {
        NSUInteger headLineIdx = [self lineIndexForPosition:[HTextPosition positionWithOffset:bindingRange.location]];
        NSUInteger tailLineIdx = [self lineIndexForPosition:[HTextPosition positionWithOffset:bindingRange.location + bindingRange.length affinity:HTextAffinityBackward]];
            ...
            if (left != CGFLOAT_MAX && right != CGFLOAT_MAX) {
                if (_container.isVerticalForm) {
                    if (fabs(point.y - left) < fabs(point.y - right)) {
                        ...
                        finalAffinity = HTextAffinityForward;
                    }else {
                        position = bindingRange.location + bindingRange.length;
                        finalAffinity = HTextAffinityBackward;
                    }
                }else {
    ...
    if (line.range.length == 0) {
        ...
        return [HTextPosition positionWithOffset:line.range.location affinity:behind ? HTextAffinityBackward:HTextAffinityForward];
    }
    ...
    if (line.range.length <= 2) {
        ...
        if (HTextIsLinebreakString(str)) { // an empty line ("\r", "\n", "\r\n")
            return [HTextPosition positionWithOffset:line.range.location];
        }
    }
    ...
    if (position >= line.range.location + line.range.length - 1) {
        if (position > line.range.location) {
            ...
            if (HTextIsLinebreakChar(c1)) {
                position--;
                if (position > line.range.location) {
                    unichar c0 = [_text.string characterAtIndex:position - 1];
                    if (HTextIsLinebreakChar(c0)) {
                        position--;
                    }
                }
            }
    ...
    if (position == line.range.location) {
        return [HTextPosition positionWithOffset:position];
    }
    if (position == line.range.location + line.range.length) {
        return [HTextPosition positionWithOffset:position affinity:HTextAffinityBackward];
    }
    ...
    if (!finalAffinityDetected) {
        ...
        if (ofs != CGFLOAT_MAX) {
            ...
            if (position >= line.range.location + line.range.length) {
                finalAffinity = RTL ? HTextAffinityForward : HTextAffinityBackward;
            } else if (position <= line.range.location) {
                finalAffinity = RTL ? HTextAffinityBackward : HTextAffinityForward;
            }else {
                finalAffinity = (ofs < (isVertical ? point.y : point.x) && !RTL) ? HTextAffinityForward : HTextAffinityBackward;
            }
        }
    }
    
    return [HTextPosition positionWithOffset:position affinity:finalAffinity];
}

- (HTextPosition *)positionForPoint:(CGPoint)point
                         oldPosition:(HTextPosition *)oldPosition
                       otherPosition:(HTextPosition *)otherPosition {
    if (!oldPosition || !otherPosition) {
        return oldPosition;
    }
    HTextPosition *newPos = [self closestPositionToPoint:point];
    ...
    HTextLine *line = _lines[lineIndex];
    HRowEdge vertical = _lineRowsEdge[line.row];
    if (_container.verticalForm) {
    ...
    if (_container.isVerticalForm) {
        if ([oldPosition compare:otherPosition] == NSOrderedAscending) { // search backward
            HTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionUp offset:1];
            if (range) return range.start;
        }else {// search forward
            HTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionDown offset:1];
            if (range) return range.end;
        }
    }else {
        if ([oldPosition compare:otherPosition] == NSOrderedAscending) { // search backward
            HTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionLeft offset:1];
            if (range) return range.start;
        }else {// search forward
            HTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionRight offset:1];
            if (range) return range.end;
        }
    }
...
- (HTextRange *)textRangeAtPoint:(CGPoint)point {
    ...
    HTextPosition *pos = [self closestPositionToPoint:point];
    if (!pos) return nil;
    ...
    if (_container.verticalForm) {
        HTextRange *range = [self textRangeByExtendingPosition:pos inDirection:(rect.origin.y >= point.y && !RTL)? UITextLayoutDirectionUp:UITextLayoutDirectionDown offset:1];
        return range;
    }else {
        HTextRange *range = [self textRangeByExtendingPosition:pos inDirection:(rect.origin.x >= point.x && !RTL)? UITextLayoutDirectionLeft:UITextLayoutDirectionRight offset:1];
        return range;
    }
}

- (HTextRange *)closestTextRangeAtPoint:(CGPoint)point {
    HTextPosition *pos = [self closestPositionToPoint:point];
    if (!pos) return nil;
    ...
    HTextRange *range = [self textRangeByExtendingPosition:pos inDirection:direction offset:1];
    return range;
}

- (HTextRange *)textRangeByExtendingPosition:(HTextPosition *)position {
    ...
    if (position.offset == visibleStart) {
        return [HTextRange rangeWithRange:NSMakeRange(position.offset, 0)];
    } else if (position.offset == visibleEnd) {
        return [HTextRange rangeWithRange:NSMakeRange(position.offset, 0) affinity:HTextAffinityBackward];
    }
    
    // binding range
    NSRange tRange;
    HTextBinding *binding = [_text attribute:HTextBindingAttributeName atIndex:position.offset longestEffectiveRange:&tRange inRange:_visibleRange];
    if (binding && tRange.length > 0 && tRange.location < position.offset) {
        return [HTextRange rangeWithRange:tRange];
    }
    ...
    if (lineIndex != NSNotFound) {
        ...
        if (emoji || seq) {
            return [HTextRange rangeWithRange:NSMakeRange(_prev, _next - _prev)];
        }
    }
    ...
    if (position.offset > visibleStart && position.offset < visibleEnd) {
        ...
        if ((c0 == '\r') && position.offset < visibleEnd) {
            ...
            if (c1 == '\n') {
                return [HTextRange rangeWithStart:[HTextPosition positionWithOffset:position.offset - 1] end:[HTextPosition positionWithOffset:position.offset + 1]];
            }
        }
        if (HTextIsLinebreakChar(c0) && position.affinity == HTextAffinityBackward) {
            NSString *str = [_text.string substringToIndex:position.offset];
            NSUInteger len = HTextLinebreakTailLength(str);
            return [HTextRange rangeWithStart:[HTextPosition positionWithOffset:position.offset - len] end:[HTextPosition positionWithOffset:position.offset]];
        }
    }
    
    return [HTextRange rangeWithRange:NSMakeRange(position.offset, 0) affinity:position.affinity];
}
...
- (HTextRange *)textRangeByExtendingPosition:(HTextPosition *)position
    ...
    if (!forwardMove && position.offset == visibleStart) {
        return [HTextRange rangeWithRange:NSMakeRange(_visibleRange.location, 0)];
    } else if (forwardMove && position.offset == visibleEnd) {
        return [HTextRange rangeWithRange:NSMakeRange(position.offset, 0) affinity:HTextAffinityBackward];
    }
    
    // extend from position
    HTextRange *fromRange = [self textRangeByExtendingPosition:position];
    if (!fromRange) return nil;
    HTextRange *allForward = [HTextRange rangeWithStart:fromRange.start end:[HTextPosition positionWithOffset:visibleEnd]];
    HTextRange *allBackward = [HTextRange rangeWithStart:[HTextPosition positionWithOffset:visibleStart] end:fromRange.end];
    ...
    if (verticalMove) { // up/down in text layout
        ...
        HTextLine *mostLeftLine = nil, *mostRightLine = nil;
        ...
        HTextLine *insideLine = _lines[insideIndex];
        NSUInteger pos;
        if (isVerticalForm) {
        ...
        HTextPosition *extPos;
        if (afinityEdge) {
            if (pos == insideLine.range.location + insideLine.range.length) {
                NSString *subStr = [_text.string substringWithRange:insideLine.range];
                NSUInteger lineBreakLen = HTextLinebreakTailLength(subStr);
                extPos = [HTextPosition positionWithOffset:pos - lineBreakLen];
            }else {
                extPos = [HTextPosition positionWithOffset:pos];
            }
        }else {
            extPos = [HTextPosition positionWithOffset:pos];
        }
        HTextRange *ext = [self textRangeByExtendingPosition:extPos];
        if (!ext) return nil;
        if (forwardMove) {
            return [HTextRange rangeWithStart:fromRange.start end:ext.end];
        }else {
            return [HTextRange rangeWithStart:ext.start end:fromRange.end];
        }
        
    }else {// left/right in text layout
        HTextPosition *toPosition = [HTextPosition positionWithOffset:position.offset + (forwardMove ? offset : -offset)];
        if (toPosition.offset <= visibleStart) return allBackward;
        else if (toPosition.offset >= visibleEnd) return allForward;
        
        HTextRange *toRange = [self textRangeByExtendingPosition:toPosition];
        ...
        return [HTextRange rangeWithRange:NSMakeRange(start, end - start)];
    }
}

- (NSUInteger)lineIndexForPosition:(HTextPosition *)position {
...
- (CGPoint)linePositionForPosition:(HTextPosition *)position {
    NSUInteger lineIndex = [self lineIndexForPosition:position];
...
- (CGRect)firstRectForRange:(HTextRange *)range {
    ...
    HTextLine *startLine = _lines[startLineIndex];
    HTextLine *endLine = _lines[endLineIndex];
    NSMutableArray *lines = [NSMutableArray new];
    for (NSUInteger i = startLineIndex; i <= startLineIndex; i++) {
        HTextLine *line = _lines[i];
    ...
    if (_container.verticalForm) {
        if (lines.count == 1) {
            ...
            if (top > bottom) HTEXT_SWAP(top, bottom);
        ...
        if (lines.count == 1) {
            ...
            if (left > right) HTEXT_SWAP(left, right);
...
- (CGRect)rectForRange:(HTextRange *)range {
    NSArray *rects = [self selectionRectsForRange:range];
    if (rects.count == 0) return CGRectNull;
    CGRect rectUnion = ((HTextSelectionRect *)rects.firstObject).rect;
    for (NSUInteger i = 1; i < rects.count; i++) {
        HTextSelectionRect *rect = rects[i];
...
- (NSArray *)selectionRectsForRange:(HTextRange *)range {
    ...
    if (startLineIndex > endLineIndex) HTEXT_SWAP(startLineIndex, endLineIndex);
    HTextLine *startLine = _lines[startLineIndex];
    HTextLine *endLine = _lines[endLineIndex];
    ...
    HTextSelectionRect *start = [HTextSelectionRect new];
    if (isVertical) {
    ...
    HTextSelectionRect *end = [HTextSelectionRect new];
    if (isVertical) {
    ...
    if (startLine.row == endLine.row) { // same row
        if (offsetStart > offsetEnd) HTEXT_SWAP(offsetStart, offsetEnd);
        HTextSelectionRect *rect = [HTextSelectionRect new];
        if (isVertical) {
        ...
        HTextSelectionRect *topRect = [HTextSelectionRect new];
        ...
        HTextSelectionRect *bottomRect = [HTextSelectionRect new];
...
- (NSArray *)selectionRectsWithoutStartAndEndForRange:(HTextRange *)range {
...
- (NSArray *)selectionRectsWithOnlyStartAndEndForRange:(HTextRange *)range {
    NSMutableArray *rects = [self selectionRectsForRange:range].mutableCopy;
    for (NSInteger i = 0, max = rects.count; i < max; i++) {
        HTextSelectionRect *rect = rects[i];
        if (!rect.containsStart && !rect.containsEnd) {
...
typedef NS_OPTIONS(NSUInteger, HTextDecorationType) {
    HTextDecorationTypeUnderline     = 1 << 0,
    HTextDecorationTypeStrikethrough = 1 << 1,
};

typedef NS_OPTIONS(NSUInteger, HTextBorderType) {
    HTextBorderTypeBackgound = 1 << 0,
    HTextBorderTypeNormal    = 1 << 1,
};

static CGRect HTextMergeRectInSameLine(CGRect rect1, CGRect rect2, BOOL isVertical) {
    if (isVertical) {
...
static void HTextGetRunsMaxMetric(CFArrayRef runs, CGFloat *xHeight, CGFloat *underlinePosition, CGFloat *lineThickness) {
...
static void HTextDrawRun(HTextLine *line, CTRunRef run, CGContextRef context, CGSize size, BOOL isVertical, NSArray *runRanges, CGFloat verticalOffset) {
    CGAffineTransform runTextMatrix = CTRunGetTextMatrix(run);
    BOOL runTextMatrixIsID = CGAffineTransformIsIdentity(runTextMatrix);
    
    CFDictionaryRef runAttrs = CTRunGetAttributes(run);
    NSValue *glyphTransformValue = CFDictionaryGetValue(runAttrs, (__bridge const void *)(HTextGlyphTransformAttributeName));
    if (!isVertical && !glyphTransformValue) { // draw run
        ...
        fillColor = HTextGetCGColor(fillColor);
        ...
        CGContextSaveGState(context); {
            ...
            if (isVertical) {
                ...
                for (HTextRunGlyphRange *oneRange in runRanges) {
                    NSRange range = oneRange.glyphRangeInRun;
                    NSUInteger rangeMax = range.location + range.length;
                    HTextRunGlyphDrawMode mode = oneRange.drawMode;
                    ...
                    for (NSUInteger g = range.location; g < rangeMax; g++) {
                        CGContextSaveGState(context); {
                            ...
                            if (mode) { // CJK glyph, need rotated
                                ...
                                if (mode == HTextRunGlyphDrawModeVerticalRotateMove) {
                                    x += w;
                                    y += w;
                                }
                                CGContextSetTextPosition(context, x, y);
                            }else {
                                CGContextRotateCTM(context, HTextDegreesToRadians(-90));
                                CGContextSetTextPosition(context,
                                                         line.position.y - size.height + glyphPositions[g].x,
                                                         line.position.x + verticalOffset + glyphPositions[g].y);
                            }
                            
                            if (HTextCTFontContainsColorBitmapGlyphs(runFont)) {
...
static void HTextSetLinePatternInContext(HTextLineStyle style, CGFloat width, CGFloat phase, CGContextRef context){
    ...
    } else if (pattern == HTextLineStylePatternCircleDot) {
...
static void HTextDrawBorderRects(CGContextRef context, CGSize size, HTextBorder *border, NSArray *rects, BOOL isVertical) {
    if (rects.count == 0) return;
    
    HTextShadow *shadow = border.shadow;
    if (shadow.color) {
        CGContextSaveGState(context);
    ...
    for (NSValue *value in rects) {
        ...
        rect = HTextCGRectPixelRound(rect);
    ...
    if (border.strokeColor && border.lineStyle > 0 && border.strokeWidth > 0) {
        ...
        HTextSetLinePatternInContext(border.lineStyle, border.strokeWidth, 0, context);
        CGFloat inset = -border.strokeWidth *0.5;
        if ((border.lineStyle & 0xFF) == HTextLineStyleThick) {
            inset *= 2;
        ...
        //------------------------- second line ------------------------------//
        if ((border.lineStyle & 0xFF) == HTextLineStyleDouble) {
            CGContextSaveGState(context);
...
static void HTextDrawLineStyle(CGContextRef context, CGFloat length, CGFloat lineWidth, HTextLineStyle style, CGPoint position, CGColorRef color, BOOL isVertical) {
    ...
    CGContextSaveGState(context); {
        if (isVertical) {
            CGFloat x, y1, y2, w;
            y1 = HTextCGFloatPixelRound(position.y);
            y2 = HTextCGFloatPixelRound(position.y + length);
            w = (styleBase == HTextLineStyleThick ? lineWidth *2 : lineWidth);
            
            CGFloat linePixel = HTextCGFloatToPixel(w);
            if (fabs(linePixel - floor(linePixel)) < 0.1) {
                int iPixel = linePixel;
                if (iPixel == 0 || (iPixel % 2)) { // odd line pixel
                    x = HTextCGFloatPixelHalf(position.x);
                }else {
                    x = HTextCGFloatPixelFloor(position.x);
                }
            }else {
                x = position.x;
            }
            ...
            HTextSetLinePatternInContext(style, lineWidth, position.y, context);
            CGContextSetLineWidth(context, w);
            if (styleBase == HTextLineStyleSingle) {
                CGContextMoveToPoint(context, x, y1);
                CGContextAddLineToPoint(context, x, y2);
                CGContextStrokePath(context);
            } else if (styleBase == HTextLineStyleThick) {
                CGContextMoveToPoint(context, x, y1);
                CGContextAddLineToPoint(context, x, y2);
                CGContextStrokePath(context);
            } else if (styleBase == HTextLineStyleDouble) {
            ...
            CGFloat x1, x2, y, w;
            x1 = HTextCGFloatPixelRound(position.x);
            x2 = HTextCGFloatPixelRound(position.x + length);
            w = (styleBase == HTextLineStyleThick ? lineWidth *2 : lineWidth);
            ...
            if (fabs(linePixel - floor(linePixel)) < 0.1) {
                int iPixel = linePixel;
                if (iPixel == 0 || (iPixel % 2)) { // odd line pixel
                    y = HTextCGFloatPixelHalf(position.y);
                }else {
                    y = HTextCGFloatPixelFloor(position.y);
                }
            }else {
                y = position.y;
            }
            ...
            CGContextSetStrokeColorWithColor(context, color);
            HTextSetLinePatternInContext(style, lineWidth, position.x, context);
            CGContextSetLineWidth(context, w);
            if (styleBase == HTextLineStyleSingle) {
...
static void HTextDrawText(HTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    CGContextSaveGState(context); {
        ...
        for (NSUInteger l = 0, lMax = lines.count; l < lMax; l++) {
            ...
            for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
                ...
                HTextDrawRun(line, run, context, size, isVertical, lineRunRanges[r], verticalOffset);
            }
...
static void HTextDrawBlockBorder(HTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    CGContextSaveGState(context);
    CGContextTranslateCTM(context, point.x, point.y);
    ...
    for (NSInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        for (NSInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            HTextBorder *border = attrs[HTextBlockBorderAttributeName];
            if (!border) continue;
            
            NSUInteger lineStartIndex = line.index;
            while (lineStartIndex > 0) {
                if (((HTextLine *)lines[lineStartIndex - 1]).row == line.row) lineStartIndex--;
                else break;
            }
            
            CGRect unionRect = CGRectZero;
            NSUInteger lineStartRow = ((HTextLine *)lines[lineStartIndex]).row;
            NSUInteger lineContinueIndex = lineStartIndex;
            NSUInteger lineContinueRow = lineStartRow;
            do {
                HTextLine *one = lines[lineContinueIndex];
                ...
                HTextLine *next = lines[lineContinueIndex + 1];
                if (next.row != lineContinueRow) {
                    HTextBorder *nextBorder = [layout.text h_attribute:HTextBlockBorderAttributeName atIndex:next.range.location];
            ...
            HTextDrawBorderRects(context, size, border, @[[NSValue valueWithCGRect:unionRect]], isVertical);
...
static void HTextDrawBorder(HTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, HTextBorderType type, BOOL (^cancel)(void)) {
    CGContextSaveGState(context);
    CGContextTranslateCTM(context, point.x, point.y);
    ...
    NSArray *lines = layout.lines;
    NSString *borderKey = (type == HTextBorderTypeNormal ? HTextBorderAttributeName : HTextBackgroundBorderAttributeName);
    ...
    for (NSInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        for (NSInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            NSDictionary *attrs = (id)CTRunGetAttributes(run);
            HTextBorder *border = attrs[borderKey];
            if (!border) continue;
            ...
            for (NSInteger ll = l; ll < lMax; ll++) {
                ...
                for (NSInteger rr = (ll == l) ? r : 0, rrMax = CFArrayGetCount(iRuns); rr < rrMax; rr++) {
                    ...
                    HTextBorder *iBorder = iAttrs[borderKey];
                    ...
                    if (isVertical) {
                        HTEXT_SWAP(iRunPosition.x, iRunPosition.y);
            ...
            for (NSInteger re = 0, reMax = runRects.count; re < reMax; re++) {
                ...
                if (isVertical) {
                    if (fabs(rect.origin.x - curRect.origin.x) < 1) {
                        curRect = HTextMergeRectInSameLine(rect, curRect, isVertical);
                    }else {
            ...
            HTextDrawBorderRects(context, size, border, drawRects, isVertical);
...
static void HTextDrawDecoration(HTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, HTextDecorationType type, BOOL (^cancel)(void)) {
    NSArray *lines = layout.lines;
    ...
    for (NSUInteger l = 0, lMax = layout.lines.count; l < lMax; l++) {
        ...
        for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            NSDictionary *attrs = (id)CTRunGetAttributes(run);
            HTextDecoration *underline = attrs[HTextUnderlineAttributeName];
            HTextDecoration *strikethrough = attrs[HTextStrikethroughAttributeName];
            
            BOOL needDrawUnderline = NO, needDrawStrikethrough = NO;
            if ((type & HTextDecorationTypeUnderline) && underline.style > 0) {
                needDrawUnderline = YES;
            }
            if ((type & HTextDecorationTypeStrikethrough) && strikethrough.style > 0) {
            ...
            if (HTextIsLinebreakString(runStr)) continue; // may need more checks...
            
            CGFloat xHeight, underlinePosition, lineThickness;
            HTextGetRunsMaxMetric(runs, &xHeight, &underlinePosition, &lineThickness);
                ...
                while (shadow) {
                    ...
                    CGContextSaveGState(context); {
                        ...
                        CGContextSaveGState(context); {
                            ...
                            HTextDrawLineStyle(context, length, thickness, underline.style, underlineStart, color, isVertical);
                        } CGContextRestoreGState(context);
                    } CGContextRestoreGState(context);
                    shadow = shadow.subShadow;
                }
            ...
            if (needDrawStrikethrough) {
                ...
                HTextDrawLineStyle(context, length, thickness, strikethrough.style, strikethroughStart, color, isVertical);
            }
        }
    }
    CGContextRestoreGState(context);
...
static void HTextDrawAttachment(HTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, UIView *targetView, CALayer *targetLayer, BOOL (^cancel)(void)) {
    
    BOOL isVertical = layout.container.verticalForm;
    CGFloat verticalOffset = isVertical ? (size.width - layout.container.size.width) : 0;
    
    for (NSUInteger i = 0, max = layout.attachments.count; i < max; i++) {
        HTextAttachment *a = layout.attachments[i];
        ...
        rect = HTextCGRectFitWithContentMode(rect, asize, a.contentMode);
        rect = HTextCGRectPixelRound(rect);
        rect = CGRectStandardize(rect);
...
static void HTextDrawShadow(HTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    ...
    CGContextSaveGState(context); {
        ...
        for (NSUInteger l = 0, lMax = layout.lines.count; l < lMax; l++) {
            ...
            for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
                ...
                NSDictionary *attrs = (id)CTRunGetAttributes(run);
                HTextShadow *shadow = attrs[HTextShadowAttributeName];
                HTextShadow *nsShadow = [HTextShadow shadowWithNSShadow:attrs[NSShadowAttributeName]]; // NSShadow compatible
                if (nsShadow) {
...
static void HTextDrawInnerShadow(HTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    CGContextSaveGState(context);
    CGContextTranslateCTM(context, point.x, point.y);
    CGContextTranslateCTM(context, 0, size.height);
    CGContextScaleCTM(context, 1, -1);
    ...
    for (NSUInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            CGContextSetTextPosition(context, linePosX, linePosY);
            NSDictionary *attrs = (id)CTRunGetAttributes(run);
            HTextShadow *shadow = attrs[HTextInnerShadowAttributeName];
            while (shadow) {
                if (!shadow.color) {
                    shadow = shadow.subShadow;
                    continue;
...
static void HTextDrawDebug(HTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, HTextDebugOption *op) {
    UIGraphicsPushContext(context);
    CGContextSaveGState(context);
    CGContextTranslateCTM(context, point.x, point.y);
    CGContextSetLineWidth(context, 1.0 / HTextScreenScale());
    ...
    if (op.CTFrameBorderColor || op.CTFrameFillColor) {
        ...
        if (!path) {
            ...
            if (op.CTFrameBorderColor) rect = HTextCGRectPixelHalf(rect);
            else rect = HTextCGRectPixelRound(rect);
    ...
    for (NSUInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        if (op.CTLineFillColor) {
            ...
            CGContextAddRect(context, HTextCGRectPixelRound(lineBounds));
            CGContextFillPath(context);
        }
        if (op.CTLineBorderColor) {
            [op.CTLineBorderColor setStroke];
            CGContextAddRect(context, HTextCGRectPixelHalf(lineBounds));
            CGContextStrokePath(context);
        }
        if (op.baselineColor) {
            [op.baselineColor setStroke];
            if (isVertical) {
                CGFloat x = HTextCGFloatPixelHalf(line.position.x);
                CGFloat y1 = HTextCGFloatPixelHalf(line.top);
                CGFloat y2 = HTextCGFloatPixelHalf(line.bottom);
                CGContextMoveToPoint(context, x, y1);
                CGContextAddLineToPoint(context, x, y2);
                CGContextStrokePath(context);
            }else {
                CGFloat x1 = HTextCGFloatPixelHalf(lineBounds.origin.x);
                CGFloat x2 = HTextCGFloatPixelHalf(lineBounds.origin.x + lineBounds.size.width);
                CGFloat y = HTextCGFloatPixelHalf(line.position.y);
                CGContextMoveToPoint(context, x1, y);
                CGContextAddLineToPoint(context, x2, y);
                CGContextStrokePath(context);
            }
        ...
        if (op.CTLineNumberColor) {
            ...
            num.h_color = op.CTLineNumberColor;
            num.h_font = [UIFont systemFontOfSize:6];
        ...
        if (op.CTRunFillColor || op.CTRunBorderColor || op.CTRunNumberColor || op.CGGlyphFillColor || op.CGGlyphBorderColor) {
            ...
            for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
                ...
                if (op.CTRunFillColor) {
                    ...
                    CGContextAddRect(context, HTextCGRectPixelRound(runTypoBounds));
                    CGContextFillPath(context);
                }
                if (op.CTRunBorderColor) {
                    [op.CTRunBorderColor setStroke];
                    CGContextAddRect(context, HTextCGRectPixelHalf(runTypoBounds));
                    CGContextStrokePath(context);
                }
                if (op.CTRunNumberColor) {
                    ...
                    num.h_color = op.CTRunNumberColor;
                    num.h_font = [UIFont systemFontOfSize:6];
                ...
                if (op.CGGlyphBorderColor || op.CGGlyphFillColor) {
                    for (NSUInteger g = 0; g < glyphCount; g++) {
                        ...
                        if (op.CGGlyphFillColor) {
                            ...
                            CGContextAddRect(context, HTextCGRectPixelRound(rect));
                            CGContextFillPath(context);
                        }
                        if (op.CGGlyphBorderColor) {
                            [op.CGGlyphBorderColor setStroke];
                            CGContextAddRect(context, HTextCGRectPixelHalf(rect));
                            CGContextStrokePath(context);
                        }
                    }
                }
...
- (void)drawInContext:(CGContextRef)context
                ...
                point:(CGPoint)point
                 view:(UIView *)view
                layer:(CALayer *)layer
                debug:(HTextDebugOption *)debug
                cancel:(BOOL (^)(void))cancel{
    @autoreleasepool {
        if (self.needDrawBlockBorder && context) {
            if (cancel && cancel()) return;
            HTextDrawBlockBorder(self, context, size, point, cancel);
        }
        if (self.needDrawBackgroundBorder && context) {
            if (cancel && cancel()) return;
            HTextDrawBorder(self, context, size, point, HTextBorderTypeBackgound, cancel);
        }
        if (self.needDrawShadow && context) {
            if (cancel && cancel()) return;
            HTextDrawShadow(self, context, size, point, cancel);
        }
        if (self.needDrawUnderline && context) {
            if (cancel && cancel()) return;
            HTextDrawDecoration(self, context, size, point, HTextDecorationTypeUnderline, cancel);
        }
        if (self.needDrawText && context) {
            if (cancel && cancel()) return;
            HTextDrawText(self, context, size, point, cancel);
        }
        if (self.needDrawAttachment && (context || view || layer)) {
            if (cancel && cancel()) return;
            HTextDrawAttachment(self, context, size, point, view, layer, cancel);
        }
        if (self.needDrawInnerShadow && context) {
            if (cancel && cancel()) return;
            HTextDrawInnerShadow(self, context, size, point, cancel);
        }
        if (self.needDrawStrikethrough && context) {
            if (cancel && cancel()) return;
            HTextDrawDecoration(self, context, size, point, HTextDecorationTypeStrikethrough, cancel);
        }
        if (self.needDrawBorder && context) {
            if (cancel && cancel()) return;
            HTextDrawBorder(self, context, size, point, HTextBorderTypeNormal, cancel);
        }
        if (debug.needDrawDebug && context) {
            if (cancel && cancel()) return;
            HTextDrawDebug(self, context, size, point, debug);
        }
    }
}
...
- (void)removeAttachmentFromViewAndLayer {
    NSAssert([NSThread isMainThread], @"This method must be called on the main thread");
    for (HTextAttachment *a in self.attachments) {
