...
@implementation HTextSimpleMarkdownParser {
    UIFont *_font;
...
- (void)initRegex {
#define HRegexp(reg, option) [NSRegularExpression regularExpressionWithPattern : @reg options : option error : NULL]
    _regexEscape = HRegexp("(\\\\\\\\|\\\\\\`|\\\\\\*|\\\\\\_|\\\\\\(|\\\\\\)|\\\\\\[|\\\\\\]|\\\\#|\\\\\\+|\\\\\\-|\\\\\\!)", 0);
    _regexHeader = HRegexp("^((\\#{1,6}[^#].*)|(\\#{6}.+))$", NSRegularExpressionAnchorsMatchLines);
...
- (void)_updateFonts {
    ...
    _boldFont = HTextFontWithBold(_font);
    _italicFont = HTextFontWithItalic(_font);
    _boldItalicFont = HTextFontWithBoldItalic(_font);
...
- (void)setColorWithBrightTheme {
    ...
    _border = [HTextBorder new];
    _border.lineStyle = HTextLineStyleSingle;
...
- (BOOL)parseText:(NSMutableAttributedString *)text selectedRange:(NSRangePointer)range {
    ...
    [_regexHeader enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        [text h_setColor:self->_controlTextColor range:NSMakeRange(r.location, whiteLen + sharpLen)];
        [text h_setColor:self->_headerTextColor range:NSMakeRange(r.location + whiteLen + sharpLen, r.length - whiteLen - sharpLen)];
    ...
    [_regexH1 enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        if (linebreak.location != NSNotFound) {
            [text h_setColor:self->_headerTextColor range:NSMakeRange(r.location, linebreak.location - r.location)];
            [text h_setFont:self->_headerFonts[0] range:NSMakeRange(r.location, linebreak.location - r.location + 1)];
            [text h_setColor:self->_controlTextColor range:NSMakeRange(linebreak.location + linebreak.length, r.location + r.length - linebreak.location - linebreak.length)];
        }
    }];
    ...
    [_regexBreakline enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        [text h_setColor:self->_controlTextColor range:result.range];
    }];
    ...
    [_regexEmphasis enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        NSRange r = result.range;
        [text h_setColor:self->_controlTextColor range:NSMakeRange(r.location, 1)];
        [text h_setColor:self->_controlTextColor range:NSMakeRange(r.location + r.length - 1, 1)];
        [text h_setFont:self->_italicFont range:NSMakeRange(r.location + 1, r.length - 2)];
    }];
    ...
    [_regexStrong enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        NSRange r = result.range;
        [text h_setColor:self->_controlTextColor range:NSMakeRange(r.location, 2)];
        [text h_setColor:self->_controlTextColor range:NSMakeRange(r.location + r.length - 2, 2)];
    ...
    [_regexStrongEmphasis enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        NSRange r = result.range;
        [text h_setColor:self->_controlTextColor range:NSMakeRange(r.location, 3)];
        [text h_setColor:self->_controlTextColor range:NSMakeRange(r.location + r.length - 3, 3)];
        [text h_setFont:self->_boldItalicFont range:NSMakeRange(r.location + 3, r.length - 6)];
    }];
    ...
    [_regexUnderline enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        [text h_setColor:self->_controlTextColor range:NSMakeRange(r.location, 2)];
        [text h_setColor:self->_controlTextColor range:NSMakeRange(r.location + r.length - 2, 2)];
        [text h_setTextUnderline:[HTextDecoration decorationWithStyle:HTextLineStyleSingle width:@1 color:nil] range:NSMakeRange(r.location + 2, r.length - 4)];
    }];
    ...
    [_regexStrikethrough enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        [text h_setColor:self->_controlTextColor range:NSMakeRange(r.location, 2)];
        [text h_setColor:self->_controlTextColor range:NSMakeRange(r.location + r.length - 2, 2)];
        [text h_setTextStrikethrough:[HTextDecoration decorationWithStyle:HTextLineStyleSingle width:@1 color:nil] range:NSMakeRange(r.location + 2, r.length - 4)];
    }];
    ...
    [_regexInlineCode enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        [text h_setColor:self->_controlTextColor range:NSMakeRange(r.location, len)];
        [text h_setColor:self->_controlTextColor range:NSMakeRange(r.location + r.length - len, len)];
        [text h_setColor:self->_inlineTextColor range:NSMakeRange(r.location + len, r.length - 2 * len)];
    ...
    [_regexLink enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        NSRange r = result.range;
        [text h_setColor:self->_linkTextColor range:r];
    }];
    ...
    [_regexLinkRefer enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        NSRange r = result.range;
        [text h_setColor:self->_controlTextColor range:r];
    }];
    ...
    [_regexCodeBlock enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        if (lenStart + lenEnd < r.length) {
            ...
            [text h_setColor:self->_codeTextColor range:codeR];
            [text h_setFont:self->_monospaceFont range:codeR];
            HTextBorder *border = [HTextBorder new];
            border.lineStyle = HTextLineStyleSingle;
            ...
            border.strokeWidth = HTextCGFloatFromPixel(2);
...
#define HLOCK(...) dispatch_semaphore_wait(_lock, DISPATCH_TIME_FOREVER); \
__VA_ARGS__; \
dispatch_semaphore_signal(_lock);

@implementation HTextSimpleEmoticonParser {
...
- (NSDictionary *)emoticonMapper {
    HLOCK(NSDictionary *mapper = _mapper); return mapper;
}
...
- (BOOL)parseText:(NSMutableAttributedString *)text selectedRange:(NSRangePointer)range {
    ...
    HLOCK(mapper = _mapper; regex = _regex;);
    ...
    for (NSUInteger i = 0, max = matches.count; i < max; i++) {
        ...
        CTFontRef font = (__bridge CTFontRef)([text h_attribute:NSFontAttributeName atIndex:oneRange.location]);
        ...
        [atr h_setTextBackedString:[HTextBackedString stringWithString:subStr] range:NSMakeRange(0, atr.length)];
        ...
        [text addAttributes:atr.h_attributes range:NSMakeRange(oneRange.location, atr.length)];
