...
/**
 The HTextParser protocol declares the required method for HTextView and HLabel
 to modify the text during editing.
 
 You can implement this protocol to add code highlighting or emoticon replacement for
 HTextView and HLabel. See `HTextSimpleMarkdownParser` and `HTextSimpleEmoticonParser` for example.
 */
@protocol HTextParser <NSObject>
@required
/**
 When text is changed in HTextView or HLabel, this method will be called.
 ...
 no selected range (such as HLabel), this value is NULL.
 ...
 */
@interface HTextSimpleMarkdownParser : NSObject <HTextParser>
 ...
 */
@interface HTextSimpleEmoticonParser : NSObject <HTextParser>
