...
static dispatch_queue_t HLabelGetReleaseQueue() {
...
@interface HLabel() <HTextDebugTarget, HTextAsyncLayerDelegate> {
    NSMutableAttributedString *_innerText; ///< nonnull
    HTextLayout *_innerLayout;
    HTextContainer *_innerContainer; ///< nonnull
    ...
    HTextLayout *_shrinkInnerLayout;
    HTextLayout *_shrinkHighlightLayout;
...
- (void)_updateLayout {
    _innerLayout = [HTextLayout layoutWithContainer:_innerContainer text:_innerText];
    _shrinkInnerLayout = [HLabel _shrinkLayoutWithLayout:_innerLayout];
}
...
- (void)_clearInnerLayout {
    ...
    HTextLayout *layout = _innerLayout;
    _innerLayout = nil;
    _shrinkInnerLayout = nil;
    dispatch_async(HLabelGetReleaseQueue(), ^{
...
- (HTextLayout *)_highlightLayout {
    return _shrinkHighlightLayout ? _shrinkHighlightLayout : _highlightLayout;
}

+ (HTextLayout *)_shrinkLayoutWithLayout:(HTextLayout *)layout {
    if (layout.text.length && layout.lines.count == 0) {
        HTextContainer *container = layout.container.copy;
...
- (void)_trackDidLongPress {
    ...
    if (_state.hasLongPressAction && _textLongPressAction) {
        ...
        HTextRange *textRange = [self._innerLayout textRangeAtPoint:point];
    ...
    if (_highlight) {
        HTextAction longPressAction = _highlight.longPressAction ? _highlight.longPressAction : _highlightLongPressAction;
        if (longPressAction) {
            HTextPosition *start = [HTextPosition positionWithOffset:_highlightRange.location];
            HTextPosition *end = [HTextPosition positionWithOffset:_highlightRange.location + _highlightRange.length affinity:HTextAffinityBackward];
            HTextRange *range = [HTextRange rangeWithStart:start end:end];
...
- (HTextHighlight *)_getHighlightAtPoint:(CGPoint)point range:(NSRangePointer)range {
    ...
    HTextHighlight *highlight = [_innerText attribute:HTextHighlightAttributeName
                                               atIndex:startIndex
...
- (void)_showHighlightAnimated:(BOOL)animated {
    ...
    if (!_highlightLayout) {
        ...
        _shrinkHighlightLayout = [HLabel _shrinkLayoutWithLayout:_highlightLayout];
...
- (void)_updateOuterTextProperties {
    _text = [_innerText h_plainTextForRange:NSMakeRange(0, _innerText.length)];
    _font = _innerText.h_font;
    if (!_font) _font = [self _defaultFont];
    _textColor = _innerText.h_color;
    if (!_textColor) _textColor = [UIColor blackColor];
    _textAlignment = _innerText.h_alignment;
    _lineBreakMode = _innerText.h_lineBreakMode;
    NSShadow *shadow = _innerText.h_shadow;
...
- (void)_initLabel {
    ((HTextAsyncLayer *)self.layer).displaysAsynchronously = NO;
    ...
    _debugOption = [HTextDebugOption sharedDebugOption];
    [HTextDebugOption addDebugTarget:self];
    
    _font = [self _defaultFont];
    _textColor = [UIColor blackColor];
    _textVerticalAlignment = HTextVerticalAlignmentCenter;
    _numberOfLines = 1;
...
- (CGSize)sizeThatFits:(CGSize)size {
    ...
    if ((!_verticalForm && size.width == self.bounds.size.width) ||
        ...
        HTextLayout *layout = self._innerLayout;
        BOOL contains = NO;
...
- (NSString *)accessibilityLabel {
    return [_innerLayout.text h_plainTextForRange:_innerLayout.text.h_rangeOfAll];
}
    ...
    if (_state.trackingTouch) {
        ...
        if (_highlight) {
            if (!_state.touchMoved || [self _getHighlightAtPoint:point range:NULL] == _highlight) {
                HTextAction tapAction = _highlight.tapAction ? _highlight.tapAction : _highlightTapAction;
                if (tapAction) {
                    HTextPosition *start = [HTextPosition positionWithOffset:_highlightRange.location];
                    HTextPosition *end = [HTextPosition positionWithOffset:_highlightRange.location + _highlightRange.length affinity:HTextAffinityBackward];
                    HTextRange *range = [HTextRange rangeWithStart:start end:end];
...
- (void)setText:(NSString *)text {
    ...
    if (needAddAttributes) {
        _innerText.h_font = _font;
        _innerText.h_color = _textColor;
        _innerText.h_shadow = [self _shadowFromProperties];
        _innerText.h_alignment = _textAlignment;
...
- (void)setTextLayout:(HTextLayout *)textLayout {
...
- (void)setDisplaysAsynchronously:(BOOL)displaysAsynchronously {
    ...
    ((HTextAsyncLayer *)self.layer).displaysAsynchronously = displaysAsynchronously;
}
...
- (CGSize)intrinsicContentSize {
    if (_preferredMaxLayoutWidth == 0) {
        HTextContainer *container = [_innerContainer copy];
        container.size = HTextContainerMaxSize;
...
#pragma mark - HTextAsyncLayerDelegate

- (HTextAsyncLayerDisplayTask *)newAsyncDisplayTask {
    
    // capture current context
    BOOL contentsNeedFade = _state.contentsNeedFade;
    NSAttributedString *text = _innerText;
    HTextContainer *container = _innerContainer;
    HTextVerticalAlignment verticalAlignment = _textVerticalAlignment;
    HTextDebugOption *debug = _debugOption;
    ...
    __block HTextLayout *layout = (_state.showingHighlight && _highlightLayout) ? self._highlightLayout : self._innerLayout;
    __block HTextLayout *shrinkLayout = nil;
    ...
    // create display task
    HTextAsyncLayerDisplayTask *task = [HTextAsyncLayerDisplayTask new];
    ...
    task.display = ^(CGContextRef context, CGSize size, BOOL (^isCancelled)(void)) {
        ...
        HTextLayout *drawLayout = layout;
        if (layoutNeedUpdate) {
            layout = [HTextLayout layoutWithContainer:container text:text];
            shrinkLayout = [HLabel _shrinkLayoutWithLayout:layout];
            if (isCancelled()) return;
            layoutUpdated = YES;
        ...
        point = HTextCGPointPixelRound(point);
    ...
    task.didDisplay = ^(CALayer *layer, BOOL finished) {
        ...
        if (!finished) {
            ...
            for (HTextAttachment *a in drawLayout.attachments) {
        ...
        __strong HLabel *view = (HLabel *)layer.delegate;
        if (!view) return;
...
@interface HLabel(IBInspectableProperties)
@end

@implementation HLabel (IBInspectableProperties)
