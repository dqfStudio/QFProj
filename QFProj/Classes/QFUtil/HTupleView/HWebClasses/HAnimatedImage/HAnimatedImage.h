...
#ifndef H_DESIGNATED_INITIALIZER
    #if __has_attribute(objc_designated_initializer)
        #define H_DESIGNATED_INITIALIZER __attribute((objc_designated_initializer))
    #else
        #define H_DESIGNATED_INITIALIZER
    #endif
#endif

extern const NSTimeInterval kHAnimatedImageDelayTimeIntervalMinimum;
...
//
@interface HAnimatedImage : NSObject
...
- (instancetype)initWithAnimatedGIFData:(NSData *)data optimalFrameCacheSize:(NSUInteger)optimalFrameCacheSize predrawingEnabled:(BOOL)isPredrawingEnabled H_DESIGNATED_INITIALIZER;
...
typedef NS_ENUM(NSUInteger, HLogLevel) {
    HLogLevelNone = 0,
    HLogLevelError,
    HLogLevelWarn,
    HLogLevelInfo,
...
@interface HAnimatedImage (Logging)
...
#define HHLog(logLevel, format, ...) [HAnimatedImage logStringFromBlock:^NSString *{ return [NSString stringWithFormat:(format), ## __VA_ARGS__]; } withLevel:(logLevel)]

@interface HWeakProxy : NSProxy
