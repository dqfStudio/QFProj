...
const NSTimeInterval kHAnimatedImageDelayTimeIntervalMinimum = 0.02;
...
@optional
- (void)debug_animatedImage:(HAnimatedImage *)animatedImage didUpdateCachedFrames:(NSIndexSet *)indexesOfFramesInCache;
- (void)debug_animatedImage:(HAnimatedImage *)animatedImage didRequestCachedFrame:(NSUInteger)index;
- (CGFloat)debug_animatedImagePredrawingSlowdownFactor:(HAnimatedImage *)animatedImage;
@end
#endif


@interface HAnimatedImage ()
...
@property (nonatomic, strong, readonly) HAnimatedImage *weakProxy;
...
- (instancetype)init {
    HAnimatedImage *animatedImage = [self initWithAnimatedGIFData:nil];
    if (!animatedImage) {
        HHLog(HLogLevelError, @"Use `-initWithAnimatedGIFData:` and supply the animated GIF data as an argument to initialize an object of type `FLAnimatedImage`.");
    }
...
- (instancetype)initWithAnimatedGIFData:(NSData *)data optimalFrameCacheSize:(NSUInteger)optimalFrameCacheSize predrawingEnabled:(BOOL)isPredrawingEnabled {
    ...
    if (!hasData) {
        HHLog(HLogLevelError, @"No animated GIF data supplied.");
        return nil;
    }
    ...
    if (self) {
        ...
        if (!_imageSource) {
            HHLog(HLogLevelError, @"Failed to `CGImageSourceCreateWithData` for animated GIF data %@", data);
            return nil;
        }
        ...
        if (!isGIFData) {
            HHLog(HLogLevelError, @"Supplied data is of type %@ and doesn't seem to be GIF data %@", imageSourceContainerType, data);
            return nil;
        }
            ...
            @autoreleasepool {
                ...
                if (frameImageRef) {
                    ...
                    if (frameImage) {
                        ...
                        if (!delayTime) {
                            if (i == 0) {
                                HHLog(HLogLevelInfo, @"Falling back to default delay time for first frame %@ because none found in GIF properties %@", frameImage, frameProperties);
                                delayTime = @(kDelayTimeIntervalDefault);
                            } else {
                                HHLog(HLogLevelInfo, @"Falling back to preceding delay time for frame %zu %@ because none found in GIF properties %@", i, frameImage, frameProperties);
                        ...
                        if ([delayTime floatValue] < ((float)kHAnimatedImageDelayTimeIntervalMinimum - FLT_EPSILON)) {
                            HHLog(HLogLevelInfo, @"Rounding frame %zu's `delayTime` from %f up to default %f (minimum supported: %f).", i, [delayTime floatValue], kDelayTimeIntervalDefault, kHAnimatedImageDelayTimeIntervalMinimum);
                            delayTime = @(kDelayTimeIntervalDefault);
                        }
                        delayTimesForIndexesMutable[@(i)] = delayTime;
                    } else {
                        skippedFrameCount++;
                        HHLog(HLogLevelInfo, @"Dropping frame %zu because valid `CGImageRef` %@ did result in `nil`-`UIImage`.", i, frameImageRef);
                    }
                    CFRelease(frameImageRef);
                } else {
                    skippedFrameCount++;
                    HHLog(HLogLevelInfo, @"Dropping frame %zu because failed to `CGImageSourceCreateImageAtIndex` with image source %@", i, self.imageSource);
                }
            }
        }
        ...
        if (self.frameCount == 0) {
            HHLog(HLogLevelInfo, @"Failed to create any valid frames for GIF with properties %@", imageProperties);
            return nil;
        } else if (self.frameCount == 1) {
            // Warn when we only have a single frame but return a valid GIF.
            HHLog(HLogLevelInfo, @"Created valid GIF but with only a single frame. Image properties: %@", imageProperties);
        } else {
        ...
        _weakProxy = (id)[HWeakProxy weakProxyForObject:self];
...
+ (instancetype)animatedImageWithGIFData:(NSData *)data {
    HAnimatedImage *animatedImage = [[HAnimatedImage alloc] initWithAnimatedGIFData:data];
    return animatedImage;
}
...
- (UIImage *)imageLazilyCachedAtIndex:(NSUInteger)index {
    ...
    if (index >= self.frameCount) {
        HHLog(HLogLevelWarn, @"Skipping requested frame %lu beyond bounds (total frame count: %lu) for animated image: %@", (unsigned long)index, (unsigned long)self.frameCount, self);
        return nil;
    }
...
- (void)addFrameIndexesToCache:(NSIndexSet *)frameIndexesToAddToCache {
    ...
    if (firstRange.length + secondRange.length != self.frameCount) {
        HHLog(HLogLevelWarn, @"Two-part frame cache range doesn't equal full range.");
    }
    ...
    HAnimatedImage * __weak weakSelf = self;
    dispatch_async(self.serialQueue, ^{
        ...
        void (^frameRangeBlock)(NSRange, BOOL *) = ^(NSRange range, BOOL *stop) {
            ...
            for (NSUInteger i = range.location; i < NSMaxRange(range); i++) {
                ...
                HHLog(HLogLevelVerbose, @"Predrew frame %lu in %f ms for animated image: %@", (unsigned long)i, (predrawDuration + slowdownDuration) * 1000, self);
#endif
...
+ (CGSize)sizeForImage:(id)image {
    ...
    } else if ([image isKindOfClass:[HAnimatedImage class]]) {
        HAnimatedImage *animatedImage = (HAnimatedImage *)image;
        imageSize = animatedImage.size;
    } else {
        // Bear trap to capture bad images; we have seen crashers cropping up on iOS 7.
        HHLog(HLogLevelError, @"`image` isn't of expected types `UIImage` or `FLAnimatedImage`: %@", image);
    }
...
- (NSMutableIndexSet *)frameIndexesToCache {
    ...
    if (self.frameCacheSizeCurrent == self.frameCount) {
        ...
        if ([indexesToCache count] != self.frameCacheSizeCurrent) {
            HHLog(HLogLevelWarn, @"Number of frames to cache doesn't equal expected cache size.");
        }
...
- (void)growFrameCacheSizeAfterMemoryWarning:(NSNumber *)frameCacheSize {
    ...
    HHLog(HLogLevelDebug, @"Grew frame cache size max to %lu after memory warning for animated image: %@", (unsigned long)self.frameCacheSizeMaxInternal, self);
...
- (void)resetFrameCacheSizeMaxInternal {
    ...
    HHLog(HLogLevelDebug, @"Reset frame cache size max (current frame cache size: %lu) for animated image: %@", (unsigned long)self.frameCacheSizeCurrent, self);
}
...
- (void)didReceiveMemoryWarning:(NSNotification *)notification {
    ...
    HHLog(HLogLevelDebug, @"Attempt setting frame cache size max to %lu (previous was %lu) after memory warning #%lu for animated image: %@", (unsigned long)FLAnimatedImageFrameCacheSizeLowMemory, (unsigned long)self.frameCacheSizeMaxInternal, (unsigned long)self.memoryWarningCount, self);
...
+ (UIImage *)predrawnImageFromImage:(UIImage *)imageToPredraw {
    ...
    if (!colorSpaceDeviceRGBRef) {
        HHLog(HLogLevelError, @"Failed to `CGColorSpaceCreateDeviceRGB` for image %@", imageToPredraw);
    ...
    // Early return on failure!
    if (!bitmapContextRef) {
        HHLog(HLogLevelError, @"Failed to `CGBitmapContextCreate` with color space %@ and parameters (width: %zu height: %zu bitsPerComponent: %zu bytesPerRow: %zu) for image %@", colorSpaceDeviceRGBRef, width, height, bitsPerComponent, bytesPerRow, imageToPredraw);
        return imageToPredraw;
    }
    ...
    // Early return on failure!
    if (!predrawnImage) {
        HHLog(HLogLevelError, @"Failed to `imageWithCGImage:scale:orientation:` with image ref %@ created with color space %@ and bitmap context %@ and properties and properties (scale: %f orientation: %ld) for image %@", predrawnImageRef, colorSpaceDeviceRGBRef, bitmapContextRef, imageToPredraw.scale, (long)imageToPredraw.imageOrientation, imageToPredraw);
        return imageToPredraw;
    }
...
@implementation HAnimatedImage (Logging)

static void (^_logBlock)(NSString *logString, HLogLevel logLevel) = nil;
static HLogLevel _logLevel;

+ (void)setLogBlock:(void (^)(NSString *logString, HLogLevel logLevel))logBlock logLevel:(HLogLevel)logLevel {
    _logBlock = logBlock;
    _logLevel = logLevel;
}

+ (void)logStringFromBlock:(NSString *(^)(void))stringBlock withLevel:(HLogLevel)level {
...
+ (instancetype)weakProxyForObject:(id)targetObject {
    HWeakProxy *weakProxy = [HWeakProxy alloc];
