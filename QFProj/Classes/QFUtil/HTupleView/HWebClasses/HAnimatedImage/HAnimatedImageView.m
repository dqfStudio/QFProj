...
@protocol HAnimatedImageViewDebugDelegate <NSObject>
@optional
- (void)debug_animatedImageView:(HAnimatedImageView *)animatedImageView waitingForFrame:(NSUInteger)index duration:(NSTimeInterval)duration;
@end
#endif


@interface HAnimatedImageView ()
...
#if defined(DEBUG) && DEBUG
@property (nonatomic, weak) id<HAnimatedImageViewDebugDelegate> debug_delegate;
#endif

@end


@implementation HAnimatedImageView
...
- (void)setAnimatedImage:(HAnimatedImage *)animatedImage {
...
- (NSTimeInterval)frameDelayGreatestCommonDivisor {
    ...
    const NSTimeInterval kGreatestCommonDivisorPrecision = 2.0 / kHAnimatedImageDelayTimeIntervalMinimum;
...
- (void)startAnimating {
    if (self.animatedImage) {
        ...
        if (!self.displayLink) {
            ...
            HWeakProxy *weakProxy = [HWeakProxy weakProxyForObject:self];
...
- (void)displayDidRefresh:(CADisplayLink *)displayLink {
    ...
    // Early return!
    if (!self.shouldAnimate) {
        HHLog(HLogLevelWarn, @"Trying to animate image when we shouldn't: %@", self);
        return;
    }
    ...
    if (delayTimeNumber) {
        ...
        if (image) {
            HHLog(HLogLevelVerbose, @"Showing frame %lu for animated image: %@", (unsigned long)self.currentFrameIndex, self.animatedImage);
            ...
            HHLog(HLogLevelDebug, @"Waiting for frame %lu for animated image: %@", (unsigned long)self.currentFrameIndex, self.animatedImage);
#if defined(DEBUG) && DEBUG
