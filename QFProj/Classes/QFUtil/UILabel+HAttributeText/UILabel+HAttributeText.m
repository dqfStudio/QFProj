...
@interface HAttributeModel : NSObject
...
@implementation HAttributeModel

@end

@implementation UILabel (HAttributeText)
...
- (id<HAttributeTextDelegate>)delegate {
    return objc_getAssociatedObject(self, _cmd);
}

- (void)setDelegate:(id<HAttributeTextDelegate>)delegate {
...
- (void)addAttributeTapActionWithStrings:(NSArray <NSString *> *)strings delegate:(id <HAttributeTextDelegate> )delegate {
    [self removeAttributeTapActions];
    [self getRangesWithStrings:strings];
    self.userInteractionEnabled = YES;
...
- (void)addAttributeTapActionWithRanges:(NSArray<NSString *> *)ranges delegate:(id<HAttributeTextDelegate>)delegate {
    [self removeAttributeTapActions];
    [self getRangesWithRanges:ranges];
    self.userInteractionEnabled = YES;
    ...
    for (CFIndex i = 0; i < count; i++) {
        ...
        if (CGRectContainsPoint(rect, point)) {
            ...
            for (int j = 0; j < link_count; j++) {
                ...
                HAttributeModel *model = self.attributeStrings[j];
